  ...
  return (
    <header className="border-b border-line bg-surface">
      <div className="mx-auto flex w-full max-w-5xl flex-wrap items-center justify-between gap-3 px-4 py-3">
        <Link
          href={`/student-dashboard/typing-game`}
        ...
        <nav aria-label={a11y("mainNavigation")} className="flex flex-wrap items-center gap-3">
          ...
          {authed ? (
            <>
              <Link
                href={`/student-dashboard/typing-game/dashboard`}
            ...
            <>
              <Link href={`/login`} className="tap-btn tap-btn-secondary tap-btn-sm">
                {nav("login")}
              </Link>
              <Link href={`/register`} className="tap-btn tap-btn-primary tap-btn-sm">
                {nav("register")}
              </Link>
            </>
          )}
