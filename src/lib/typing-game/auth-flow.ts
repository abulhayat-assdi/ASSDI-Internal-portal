...
export function landingForRoles(roles: AppRole[], locale: Locale): string {
  if (roles.includes("super_admin")) return `/dashboard/typing-game/super-admin`;
  if (roles.includes("admin")) return `/dashboard/typing-game/admin`;
  if (roles.includes("teacher")) return `/dashboard/typing-game/teacher`;
  return `/student-dashboard/typing-game/dashboard`;
}
