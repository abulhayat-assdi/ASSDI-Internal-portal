...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ shareSlug: string }> }) {
    ...
    // Public share link — no course context, super-admin bypass to look up
    // the globally-unique shareSlug (see cv/public/[shareSlug]/route.ts).
    return withCourseContext({ courseId: null, isSuperAdmin: true }, async (tx) => {
        const draft = await tx.cvDraft.findFirst({
        ...
        try {
            ...
            console.error("[PUBLIC PDF Route Error]", err);
