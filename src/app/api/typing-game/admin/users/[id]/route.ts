...
export async function GET(
  ...
  try {
    const detail = await ac.store.getUserDetail((await ctx.params).id);
    if (!detail) throw new ForbiddenError();
    const memberships = await ac.store.membershipsOf((await ctx.params).id);
...
export async function PATCH(
  ...
  try {
    ...
    if (
      ...
      await ac.store.updateAccountStatus((await ctx.params).id, body.status);
      return NextResponse.json({ ok: true });
    }
    throw new ForbiddenError();
  } catch (e) {
