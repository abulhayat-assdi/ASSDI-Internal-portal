...
import type { DraftPatch } from "@/lib/typing-game/server/competition-store";
...
export async function GET(_req: NextRequest, ctx: Ctx): Promise<Response> {
  ...
  return handleGetCompetition((await ctx.params).id, c);
}

export async function PATCH(req: NextRequest, ctx: Ctx): Promise<Response> {
  const c = await staffCompetitionContext();
  if (c instanceof NextResponse) return c;
  return handleUpdateDraft((await ctx.params).id, await readJson(req), c);
}
