...
CREATE OR REPLACE FUNCTION typing_game.fn_create_boss(p_def jsonb)
RETURNS uuid
...
BEGIN
  ...
  END IF;
  INSERT INTO typing_game.boss_definitions (
...
CREATE OR REPLACE FUNCTION typing_game.fn_update_boss_draft(
...
BEGIN
  IF NOT typing_game.fn_is_mission_admin() AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM typing_game.boss_definitions
                 WHERE id = p_boss AND status = 'draft') THEN
    RAISE EXCEPTION 'NOT_DRAFT';
  END IF;
  UPDATE typing_game.boss_definitions SET
    ...
    updated_at = now()
  WHERE id = p_boss;
  INSERT INTO typing_game.boss_versions (boss_id, version, definition)
  SELECT p_boss, version,
    to_jsonb(d.*) - 'created_at' - 'updated_at'
  FROM typing_game.boss_definitions d WHERE d.id = p_boss;
END;
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_add_boss_phase(
...
BEGIN
  ...
  IF p_hp_from IS NULL OR p_hp_to IS NULL OR p_hp_from <= p_hp_to THEN
    RAISE EXCEPTION 'MALFORMED';
  END IF;
  INSERT INTO typing_game.boss_phases
...
CREATE OR REPLACE FUNCTION typing_game.fn_set_boss_status(
  p_boss uuid, p_status typing_game.boss_status
)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
...
BEGIN
  IF NOT typing_game.fn_is_mission_admin() AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  UPDATE typing_game.boss_definitions SET status = p_status, updated_at = now()
  WHERE id = p_boss;
  IF NOT FOUND THEN
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_create_boss_instance(
...
BEGIN
  ...
  -- caller's admin role is scoped to p_clan's own organization.
  -- fn_can_manage_clan(p_clan) already performs that org-scoped check.
  IF NOT typing_game.fn_can_manage_clan(p_clan)
    AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  SELECT max_hp INTO v_max FROM typing_game.boss_definitions
  WHERE id = p_boss AND status = 'active';
  IF NOT FOUND THEN
    RAISE EXCEPTION 'BOSS_INACTIVE';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM typing_game.clans
  ...
  IF p_start IS NULL OR p_end IS NULL OR p_end <= p_start THEN
    RAISE EXCEPTION 'MALFORMED';
  END IF;
  INSERT INTO typing_game.boss_instances
  ...
  RETURNING id INTO v_id;
  IF NOT FOUND THEN
    SELECT id INTO v_id FROM typing_game.boss_instances
...
CREATE OR REPLACE FUNCTION typing_game.fn_activate_boss_instance(p_instance uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  ...
  v_slug text;
BEGIN
  SELECT * INTO v_inst FROM typing_game.boss_instances WHERE id = p_instance;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  -- fn_is_mission_admin() alone cannot gate this: it does not verify the
  -- caller's admin role is scoped to this instance's clan's organization.
  -- fn_can_manage_clan(...) already performs that org-scoped check.
  IF NOT typing_game.fn_can_manage_clan(v_inst.clan_id)
    AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  ...
  FROM typing_game.boss_definitions WHERE id = v_inst.boss_id;
  INSERT INTO typing_game.boss_participants
  ...
  SELECT p_instance, m.user_id, true,
         jsonb_build_object('level', COALESCE(p.current_level, 1),
                            'clan_id', m.clan_id)
  FROM typing_game.clan_members m
  JOIN typing_game.profiles p ON p.id = m.user_id
  WHERE m.clan_id = v_inst.clan_id AND m.status = 'active'
    AND COALESCE(p.current_level, 1) >= v_min
  ON CONFLICT (instance_id, user_id) DO NOTHING;
  FOR v_slug IN
    SELECT DISTINCT x FROM typing_game.boss_phases ph,
      jsonb_array_elements_text(
        COALESCE(ph.game_constraints -> 'games', '[]'::jsonb)) AS x
    WHERE ph.boss_id = v_inst.boss_id
  LOOP
    INSERT INTO typing_game.boss_games (instance_id, game_id)
    SELECT p_instance, g.id FROM typing_game.games g
    WHERE g.slug = v_slug AND g.is_active
    ON CONFLICT DO NOTHING;
  END LOOP;
  UPDATE typing_game.boss_instances
  SET status = 'active', current_phase = 0 WHERE id = p_instance;
  PERFORM typing_game.fn_log_clan(v_inst.clan_id, 'boss_started', auth.uid(),
...
CREATE OR REPLACE FUNCTION typing_game.fn_finalize_boss(p_instance uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  ...
  v_coins int;
BEGIN
  SELECT * INTO v_inst FROM typing_game.boss_instances
  ...
  IF v_inst.status NOT IN ('defeated', 'expired') THEN
    RAISE EXCEPTION 'INVALID_STATE';
  END IF;
  SELECT * INTO v_def FROM typing_game.boss_definitions
  ...
  UPDATE typing_game.boss_instances SET status = 'processing'
  WHERE id = p_instance;
  SELECT COALESCE(sum(damage), 0), count(DISTINCT user_id)
    INTO v_total, v_count
  FROM typing_game.boss_participants WHERE instance_id = p_instance AND damage > 0;
  SELECT user_id, damage INTO v_top, v_top_dmg
  FROM typing_game.boss_participants WHERE instance_id = p_instance
  ORDER BY damage DESC LIMIT 1;
  INSERT INTO typing_game.boss_results
  ...
  FOR v_uid IN SELECT user_id FROM typing_game.boss_participants
  ...
  LOOP
    ...
    IF v_outcome = 'defeated'
       AND EXISTS (SELECT 1 FROM typing_game.boss_participants
  ...
  PERFORM typing_game.fn_log_clan(v_inst.clan_id, 'boss_finalized', auth.uid(),
...
CREATE OR REPLACE FUNCTION typing_game.fn_pay_boss_reward(
...
BEGIN
  ...
  IF p_xp > 0 THEN
    ...
    FROM typing_game.profiles WHERE id = p_user
    ON CONFLICT (user_id, source, reference_id) DO NOTHING;
    UPDATE typing_game.profiles SET xp_total = xp_total + p_xp,
  ...
  IF p_coins > 0 THEN
    ...
    FROM typing_game.profiles WHERE id = p_user
    ON CONFLICT (user_id, source, reference_id) DO NOTHING;
    UPDATE typing_game.profiles SET coin_balance = coin_balance + p_coins
    WHERE id = p_user AND EXISTS (
...
CREATE OR REPLACE FUNCTION typing_game.fn_boss_sweep()
RETURNS int
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  ...
  v_n int := 0;
BEGIN
  FOR r IN SELECT id FROM typing_game.boss_instances WHERE status = 'scheduled'
  LOOP
    BEGIN
      PERFORM typing_game.fn_activate_boss_sweep(r.id);
      v_n := v_n + 1;
    EXCEPTION WHEN OTHERS THEN
      NULL;
    END;
  END LOOP;
  FOR r IN SELECT id FROM typing_game.boss_instances
           WHERE status = 'active' AND end_at <= now()
  LOOP
    UPDATE typing_game.boss_instances SET status = 'expired' WHERE id = r.id;
    PERFORM typing_game.fn_finalize_boss(r.id);
    v_n := v_n + 1;
  END LOOP;
  RETURN v_n;
END;
...
CREATE OR REPLACE FUNCTION typing_game.fn_activate_boss_sweep(p_instance uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  UPDATE typing_game.boss_instances
  SET status = 'active', current_phase = 0 WHERE id = p_instance;
  PERFORM typing_game.fn_log_clan(v_inst.clan_id, 'boss_started', NULL,
    jsonb_build_object('instance_id', p_instance));
END;
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_advance_boss(p_instance uuid)
RETURNS typing_game.boss_instance_status
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
...
BEGIN
  ...
  END IF;
  IF v_inst.status = 'scheduled' THEN
    PERFORM typing_game.fn_activate_boss_sweep(p_instance);
  ELSIF v_inst.status = 'active' AND v_inst.end_at <= now() THEN
    UPDATE typing_game.boss_instances SET status = 'expired'
    WHERE id = p_instance;
    PERFORM typing_game.fn_finalize_boss(p_instance);
  END IF;
  SELECT status INTO v_inst.status FROM typing_game.boss_instances
  WHERE id = p_instance;
...
CREATE OR REPLACE FUNCTION typing_game.fn_boss_state(p_instance uuid)
RETURNS jsonb
...
BEGIN
  SELECT * INTO v_inst FROM typing_game.boss_instances WHERE id = p_instance;
  IF NOT FOUND OR NOT typing_game.fn_is_boss_viewer(p_instance) THEN
    RETURN NULL;
  END IF;
  SELECT * INTO v_def FROM typing_game.boss_definitions WHERE id = v_inst.boss_id;
  SELECT jsonb_build_object(
    ...
    'phases', (SELECT COALESCE(jsonb_agg(
      ...
      FROM typing_game.boss_phases WHERE boss_id = v_inst.boss_id),
    'mine', (SELECT jsonb_build_object('damage', damage,
        'attempts_used', attempts_used)
      FROM typing_game.boss_participants
    ...
    'top', (SELECT COALESCE(jsonb_agg(t.* ORDER BY t.damage DESC), '[]'::jsonb)
      FROM (SELECT COALESCE(NULLIF(p.full_name, ''), 'Player') AS name,
            ...
            FROM typing_game.boss_participants bp
            JOIN typing_game.profiles p ON p.id = bp.user_id
    ...
    'feed', (SELECT COALESCE(jsonb_agg(f.* ORDER BY f.created_at DESC), '[]'::jsonb)
      FROM (SELECT e.kind, e.summary, e.created_at
            FROM typing_game.clan_activity e
...
FROM typing_game.boss_definitions d
JOIN (VALUES
...
CREATE OR REPLACE FUNCTION typing_game.fn_boss_damage_for(
...
CREATE OR REPLACE FUNCTION typing_game.fn_submit_boss_attempt(
...
BEGIN
  ...
  END IF;
  SELECT * INTO v_part FROM typing_game.boss_participants
  WHERE instance_id = p_instance AND user_id = v_me;
  ...
  SELECT * INTO v_phase FROM typing_game.boss_phases
  ...
  IF FOUND THEN
    ...
    IF v_games <> '{}' AND NOT EXISTS (
        SELECT 1 FROM typing_game.games g
    ...
    IF (v_phase.rules ->> 'min_accuracy')::numeric IS NOT NULL THEN
      SELECT * INTO v_res FROM typing_game.attempt_results
  ...
  v_key := 'boss:' || p_instance::text || ':attempt:' || p_attempt::text
    || ':damage:v1';
  INSERT INTO typing_game.boss_damage_events
  ...
  END IF;
  v_damage := typing_game.fn_boss_damage_for(
    v_def.scoring_profile, v_res.score, v_res.effective_wpm,
    v_res.accuracy, COALESCE(v_phase.damage_multiplier, 1));
  UPDATE typing_game.boss_damage_events SET damage = v_damage, phase = v_inst.current_phase
  WHERE key = v_key;
  INSERT INTO typing_game.boss_attempts
  ...
  SET current_hp = v_new_hp, current_phase = v_new_phase,
      status = CASE WHEN v_new_hp = 0 THEN 'defeated'::typing_game.boss_instance_status
                    ELSE status END,
      defeated_at = CASE WHEN v_new_hp = 0 THEN now() ELSE defeated_at END
  WHERE id = p_instance;
  IF v_new_phase <> v_inst.current_phase THEN
    INSERT INTO typing_game.boss_phase_events
      (instance_id, from_phase, to_phase, hp_remaining)
    VALUES (p_instance, v_inst.current_phase, v_new_phase, v_new_hp);
    PERFORM typing_game.fn_log_clan(v_inst.clan_id, 'boss_phase', v_me,
      jsonb_build_object('instance_id', p_instance,
        'from_phase', v_inst.current_phase, 'to_phase', v_new_phase));
  END IF;
  IF v_new_hp = 0 AND v_inst.current_hp > 0 THEN
    PERFORM typing_game.fn_log_clan(v_inst.clan_id, 'boss_defeated', v_me,
      jsonb_build_object('instance_id', p_instance));
  END IF;
  UPDATE typing_game.boss_participants
  SET damage = damage + v_damage, attempts_used = attempts_used + 1
  WHERE instance_id = p_instance AND user_id = v_me;
  PERFORM typing_game.fn_log_clan(v_inst.clan_id, 'boss_damage', v_me,
    jsonb_build_object('instance_id', p_instance, 'damage', v_damage));
  RETURN v_damage;
END;
$$;

REVOKE ALL ON FUNCTION typing_game.fn_create_boss(jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_create_boss(jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_update_boss_draft(uuid, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_update_boss_draft(uuid, jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_add_boss_phase(uuid, int, text, bigint, bigint, jsonb, numeric, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_add_boss_phase(uuid, int, text, bigint, bigint, jsonb, numeric, jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_set_boss_status(uuid, typing_game.boss_status) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_set_boss_status(uuid, typing_game.boss_status) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_create_boss_instance(uuid, uuid, timestamptz, timestamptz, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_create_boss_instance(uuid, uuid, timestamptz, timestamptz, int) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_activate_boss_instance(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_activate_boss_instance(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_submit_boss_attempt(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_submit_boss_attempt(uuid, uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_finalize_boss(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_finalize_boss(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_advance_boss(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_advance_boss(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_boss_sweep() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_boss_sweep() TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_boss_state(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_boss_state(uuid) TO authenticated;
