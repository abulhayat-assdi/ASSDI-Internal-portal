...
CREATE OR REPLACE FUNCTION typing_game.fn_competition_leaderboard(p_competition uuid)
RETURNS TABLE (
  rank int,
  display_name text,
  batch_title text,
...
BEGIN
  ...
  IF v_status <> 'finalized'
    AND NOT typing_game.fn_can_manage_competition(p_competition)
    AND NOT EXISTS (
  ...
  FROM typing_game.competition_results r
  LEFT JOIN typing_game.competition_entries e
    ON e.competition_id = r.competition_id AND e.user_id = r.ref_id
  LEFT JOIN typing_game.profiles p ON p.id = r.ref_id
  LEFT JOIN typing_game.batches b ON b.id = e.batch_id
