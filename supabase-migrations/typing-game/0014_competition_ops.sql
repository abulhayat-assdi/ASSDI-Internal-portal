...
CREATE OR REPLACE FUNCTION typing_game.fn_create_competition(
  p_slug text,
  p_title text,
  p_description text,
  p_type typing_game.competition_type,
  p_visibility text,
  p_batches uuid[],
  p_courses uuid[],
  p_skill_bands text[],
...
BEGIN
  ...
  INSERT INTO typing_game.competitions
  ...
  FOR v_slug IN SELECT * FROM unnest(COALESCE(p_games, '{}')) LOOP
    SELECT id INTO v_gid FROM typing_game.games
    ...
    IF p_game_versions IS NOT NULL AND (p_game_versions ? v_slug) THEN
      SELECT id INTO v_ver FROM typing_game.game_versions
      WHERE game_id = v_gid AND version = (p_game_versions ->> v_slug)::int;
      IF NOT FOUND THEN
        RAISE EXCEPTION 'VERSION_UNKNOWN';
      END IF;
    END IF;
    INSERT INTO typing_game.competition_games (competition_id, game_id, game_version_id)
    VALUES (v_id, v_gid, v_ver);
  END LOOP;

  IF NOT typing_game.fn_can_manage_competition(v_id) THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;

  INSERT INTO typing_game.competition_state_events (competition_id, actor_user_id, to_status)
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_update_competition_draft(
  p_id uuid,
  p_patch jsonb
)
RETURNS void
...
AS $$
DECLARE
  v_status typing_game.competition_status;
  v_slug text;
  v_gid uuid;
BEGIN
  IF NOT typing_game.fn_can_manage_competition(p_id) THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  SELECT status INTO v_status FROM typing_game.competitions WHERE id = p_id;
  IF NOT FOUND THEN
  ...
  UPDATE typing_game.competitions SET
  ...
  IF p_patch ? 'game_slugs' THEN
    DELETE FROM typing_game.competition_games WHERE competition_id = p_id;
    FOR v_slug IN SELECT * FROM jsonb_array_elements_text(p_patch -> 'game_slugs') LOOP
      ...
      IF NOT FOUND THEN
        RAISE EXCEPTION 'GAME_UNKNOWN';
      END IF;
      INSERT INTO typing_game.competition_games (competition_id, game_id)
      VALUES (p_id, v_gid);
    END LOOP;
  END IF;
END;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_transition_competition(
  p_id uuid,
  p_to typing_game.competition_status
)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
...
DECLARE
  v_me uuid := auth.uid();
  v_from typing_game.competition_status;
  v_row typing_game.competitions%ROWTYPE;
  v_ok boolean := false;
BEGIN
  IF v_me IS NULL THEN
    RAISE EXCEPTION 'NOT_AUTHENTICATED';
  ...
  IF NOT typing_game.fn_can_manage_competition(p_id) THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  SELECT * INTO v_row FROM typing_game.competitions WHERE id = p_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  v_from := v_row.status;
  ...
  UPDATE typing_game.competitions SET status = p_to WHERE id = p_id;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_register_entry(p_competition uuid)
RETURNS uuid
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  SELECT * INTO v_prof FROM typing_game.profiles WHERE id = v_me;
  IF NOT FOUND OR v_prof.account_status <> 'active' THEN
    RAISE EXCEPTION 'ACCOUNT_INACTIVE';
  END IF;

  IF EXISTS (
    SELECT 1 FROM typing_game.competition_entries
  ...
  INSERT INTO typing_game.competition_entries
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_attach_attempt(
  p_competition uuid,
  p_attempt_id uuid
)
RETURNS uuid
...
DECLARE
  v_me uuid := auth.uid();
  v_comp typing_game.competitions%ROWTYPE;
  v_entry typing_game.competition_entries%ROWTYPE;
...
BEGIN
  ...
  SELECT * INTO v_entry FROM typing_game.competition_entries
  WHERE competition_id = p_competition AND user_id = v_me;
  IF NOT FOUND THEN
  ...
  SELECT * INTO v_attempt FROM typing_game.game_attempts WHERE id = p_attempt_id;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM typing_game.competition_games
  ...
  SELECT count(*) INTO v_count FROM typing_game.competition_attempts
  ...
  IF EXISTS (
    SELECT 1 FROM typing_game.competition_attempts WHERE attempt_id = p_attempt_id
  ) THEN
  ...
  INSERT INTO typing_game.competition_attempts (competition_id, entry_id, attempt_id)
