...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.reward_profiles (
  id text PRIMARY KEY,
...
INSERT INTO typing_game.reward_profiles
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.levels (
...
INSERT INTO typing_game.levels (level, required_xp, title_en, title_bn) VALUES
  (1, 0, 'Newcomer', 'নবাগত'),
  (2, 30, 'Rookie', 'নবীন'),
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.xp_ledger (
...
CREATE INDEX IF NOT EXISTS xp_ledger_user_idx
  ON typing_game.xp_ledger (user_id, created_at DESC);

CREATE TABLE IF NOT EXISTS typing_game.coin_ledger (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  amount int NOT NULL CHECK (amount <> 0),
  source text NOT NULL,
...
CREATE INDEX IF NOT EXISTS coin_ledger_user_idx
  ON typing_game.coin_ledger (user_id, created_at DESC);

-- ---------------------------------------------------------------------------
-- Badges: definitions (machine-evaluable criteria) vs one-time awards.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.badges (
...
INSERT INTO typing_game.badges
...
CREATE TABLE IF NOT EXISTS typing_game.badge_awards (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  badge_id text NOT NULL REFERENCES typing_game.badges (id) ON DELETE RESTRICT,
  attempt_id uuid REFERENCES typing_game.game_attempts (id) ON DELETE SET NULL,
  awarded_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (user_id, badge_id)
);
CREATE INDEX IF NOT EXISTS badge_awards_user_idx ON typing_game.badge_awards (user_id);

-- ---------------------------------------------------------------------------
-- Achievements: milestone records (thresholds over lifetime stats).
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.achievements (
  id text PRIMARY KEY CHECK (char_length(id) BETWEEN 1 AND 80),
  name_en text NOT NULL,
  name_bn text NOT NULL DEFAULT '',
  description_en text NOT NULL DEFAULT '',
...
INSERT INTO typing_game.achievements (id, name_en, metric, threshold) VALUES
...
CREATE TABLE IF NOT EXISTS typing_game.achievement_awards (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  achievement_id text NOT NULL REFERENCES typing_game.achievements (id) ON DELETE RESTRICT,
  value numeric NOT NULL,
  awarded_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (user_id, achievement_id)
);
CREATE INDEX IF NOT EXISTS achievement_awards_user_idx
  ON typing_game.achievement_awards (user_id);

-- ---------------------------------------------------------------------------
-- Streaks: one row per user + one row per qualifying day (idempotent days).
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.streaks (
...
CREATE TABLE IF NOT EXISTS typing_game.streak_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS streak_events_user_idx ON typing_game.streak_events (user_id);

-- ---------------------------------------------------------------------------
-- Personal records (best per game × metric; ties keep the older attempt).
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.personal_records (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  game_id uuid NOT NULL REFERENCES typing_game.games (id) ON DELETE CASCADE,
  metric text NOT NULL CHECK (metric IN (
    'best_wpm', 'best_accuracy', 'best_score', 'fastest_ms', 'most_chars')),
  value numeric NOT NULL,
  attempt_id uuid NOT NULL REFERENCES typing_game.game_attempts (id) ON DELETE CASCADE,
  recorded_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (user_id, game_id, metric)
);
CREATE INDEX IF NOT EXISTS personal_records_user_idx
  ON typing_game.personal_records (user_id);

-- ---------------------------------------------------------------------------
-- Unlock cache (evaluated state; first-unlock time preserved).
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.game_unlocks (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.reward_events (
...
-- ---------------------------------------------------------------------------
ALTER TABLE typing_game.reward_profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.levels ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.badges ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.achievements ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.xp_ledger ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.coin_ledger ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.badge_awards ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.achievement_awards ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.streaks ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.streak_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.personal_records ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.game_unlocks ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.reward_events ENABLE ROW LEVEL SECURITY;

CREATE POLICY reward_profiles_select_public ON typing_game.reward_profiles
  FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY levels_select_public ON typing_game.levels
  FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY badges_select_public ON typing_game.badges
  FOR SELECT TO anon, authenticated USING (is_active);
CREATE POLICY achievements_select_public ON typing_game.achievements
  FOR SELECT TO anon, authenticated USING (is_active);

CREATE POLICY xp_ledger_select_own ON typing_game.xp_ledger
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY coin_ledger_select_own ON typing_game.coin_ledger
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY badge_awards_select_own ON typing_game.badge_awards
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY achievement_awards_select_own ON typing_game.achievement_awards
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY streaks_select_own ON typing_game.streaks
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY streak_events_select_own ON typing_game.streak_events
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY personal_records_select_own ON typing_game.personal_records
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY game_unlocks_select_own ON typing_game.game_unlocks
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY reward_events_select_own ON typing_game.reward_events
  FOR SELECT TO authenticated USING (user_id = auth.uid());
