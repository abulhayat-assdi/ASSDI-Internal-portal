...
CREATE OR REPLACE FUNCTION typing_game.fn_create_mission(p_def jsonb)
RETURNS uuid
...
BEGIN
  ...
  END IF;
  INSERT INTO typing_game.missions (
  ...
  INSERT INTO typing_game.mission_versions (mission_id, version, definition)
  SELECT v_id, version,
    to_jsonb(m.*) - 'created_at' - 'updated_at'
  FROM typing_game.missions m WHERE m.id = v_id;
  RETURN v_id;
...
CREATE OR REPLACE FUNCTION typing_game.fn_update_mission_draft(
...
AS $$
DECLARE
  v_status typing_game.mission_status;
BEGIN
  IF NOT typing_game.fn_is_mission_admin() AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  SELECT status INTO v_status FROM typing_game.missions WHERE id = p_mission;
  IF NOT FOUND THEN
  ...
  END IF;
  UPDATE typing_game.missions SET
  ...
  SELECT p_mission, version,
    to_jsonb(m.*) - 'created_at' - 'updated_at'
  FROM typing_game.missions m WHERE m.id = p_mission;
END;
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_set_mission_status(
  p_mission uuid, p_status typing_game.mission_status
)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
...
BEGIN
  IF NOT typing_game.fn_is_mission_admin() AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  UPDATE typing_game.missions SET status = p_status, updated_at = now()
  WHERE id = p_mission;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_assign_daily_missions(p_user uuid)
RETURNS int
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  LOOP
    ...
    IF NOT FOUND THEN
      SELECT id INTO v_inst FROM typing_game.mission_instances
      WHERE mission_id = v_m.id AND user_id = p_user AND period_start = v_day;
    END IF;
    INSERT INTO typing_game.daily_mission_assignments
...
CREATE OR REPLACE FUNCTION typing_game.fn_assign_weekly(p_user uuid)
RETURNS int
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  LOOP
    ...
    END IF;
    INSERT INTO typing_game.weekly_challenges
...
CREATE OR REPLACE FUNCTION typing_game.fn_add_mission_objective(
...
BEGIN
  ...
  IF p_position IS NULL THEN
    ...
    FROM typing_game.mission_objectives WHERE mission_id = p_mission;
  ELSE
    v_pos := p_position;
  END IF;
  INSERT INTO typing_game.mission_objectives (mission_id, position, kind, target)
...
REVOKE ALL ON FUNCTION typing_game.fn_add_mission_objective(uuid, text, jsonb, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_add_mission_objective(uuid, text, jsonb, int) TO authenticated;

-- Opt-in to an EVENT mission (self only; start() still gates prerequisites).
CREATE OR REPLACE FUNCTION typing_game.fn_join_mission(p_mission uuid)
RETURNS uuid
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  IF v_me IS NULL THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  SELECT id INTO v_inst FROM typing_game.missions
  ...
  INSERT INTO typing_game.mission_instances
    (mission_id, user_id, period, period_start)
  VALUES (p_mission, v_me, 'event', (now() AT TIME ZONE typing_game.fn_mission_tz(v_me))::date)
  ON CONFLICT (mission_id, user_id, period_start) DO NOTHING
  RETURNING id INTO v_inst;
  IF NOT FOUND THEN
    SELECT id INTO v_inst FROM typing_game.mission_instances
    WHERE mission_id = p_mission AND user_id = v_me
      AND period_start = (now() AT TIME ZONE typing_game.fn_mission_tz(v_me))::date;
  END IF;
  RETURN v_inst;
END;
$$;

REVOKE ALL ON FUNCTION typing_game.fn_join_mission(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_join_mission(uuid) TO authenticated;

CREATE OR REPLACE FUNCTION typing_game.fn_start_mission(p_instance uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  FROM typing_game.mission_instances WHERE id = p_instance;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  ...
  END IF;
  v_elig := typing_game.fn_mission_eligibility(v_mid, v_user);
