...
CREATE OR REPLACE FUNCTION typing_game.fn_require_shop_admin()
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
BEGIN
  IF typing_game.is_super_admin() OR typing_game.fn_is_mission_admin() THEN
    RETURN;
  END IF;
...
CREATE OR REPLACE FUNCTION typing_game.shop_equip_slot(p_category text)
RETURNS text
...
CREATE OR REPLACE FUNCTION typing_game.fn_create_shop_item(p_def jsonb)
RETURNS uuid
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  v_id uuid;
BEGIN
  PERFORM typing_game.fn_require_shop_admin();
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM typing_game.shop_categories
  ...
  IF NULLIF(p_def ->> 'preview_key', '') IS NOT NULL
    ...
    RAISE EXCEPTION 'MALFORMED';
  END IF;
  INSERT INTO typing_game.shop_items (
  ...
  INSERT INTO typing_game.shop_item_versions (item_id, version, definition)
  SELECT v_id, version, to_jsonb(t.*) - 'created_at' - 'updated_at'
  FROM typing_game.shop_items t WHERE t.id = v_id;
  RETURN v_id;
...
CREATE OR REPLACE FUNCTION typing_game.fn_update_shop_item(
...
BEGIN
  PERFORM typing_game.fn_require_shop_admin();
  IF NOT EXISTS (SELECT 1 FROM typing_game.shop_items
                 WHERE id = p_item AND NOT is_active) THEN
    RAISE EXCEPTION 'NOT_DRAFT';
  END IF;
  UPDATE typing_game.shop_items SET
    ...
    updated_at = now()
  WHERE id = p_item;
  IF (SELECT price_coins FROM typing_game.shop_items WHERE id = p_item) < 0 THEN
    RAISE EXCEPTION 'MALFORMED';
  END IF;
  INSERT INTO typing_game.shop_item_versions (item_id, version, definition)
  SELECT p_item, version, to_jsonb(t.*) - 'created_at' - 'updated_at'
  FROM typing_game.shop_items t WHERE t.id = p_item;
END;
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_set_item_active(
...
BEGIN
  PERFORM typing_game.fn_require_shop_admin();
  UPDATE typing_game.shop_items SET is_active = p_active, updated_at = now()
  WHERE id = p_item;
  IF NOT FOUND THEN
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_purchase_item(
...
BEGIN
  ...
  IF p_request IS NULL OR char_length(p_request) NOT BETWEEN 1 AND 80 THEN
    RAISE EXCEPTION 'MALFORMED';
  END IF;
  SELECT * INTO v_item FROM typing_game.shop_items WHERE id = p_item;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  ...
  IF p_clan IS NOT NULL THEN
    IF v_item.item_type <> 'clan_cosmetic' THEN
      RAISE EXCEPTION 'MALFORMED';
    END IF;
    IF NOT typing_game.fn_is_clan_staff(p_clan, v_me) THEN
      RAISE EXCEPTION 'FORBIDDEN';
    END IF;
  ...
  SELECT id INTO v_existing FROM typing_game.purchase_records
  ...
  END IF;
  IF p_clan IS NOT NULL THEN
    PERFORM 1 FROM typing_game.clans WHERE id = p_clan FOR UPDATE;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'NOT_FOUND';
    END IF;
  END IF;
  ...
  IF v_item.purchase_limit_total IS NOT NULL AND (
      SELECT count(*) FROM typing_game.purchase_records
  ...
  IF p_clan IS NULL THEN
    ...
    WHERE user_id = v_me AND item_id = p_item;
  ELSE
    SELECT COALESCE(quantity, 0) INTO v_have FROM typing_game.clan_inventory
  ...
  END IF;
  INSERT INTO typing_game.purchase_records
  ...
  VALUES (v_key, v_me, p_clan, p_item, 1,
    ...
    jsonb_build_object('authorizer_role',
      CASE WHEN p_clan IS NULL THEN 'owner'
           ELSE (SELECT role::text FROM typing_game.clan_members
  ...
  IF NOT FOUND THEN
    ...
    SELECT id INTO v_pid FROM typing_game.purchase_records
    WHERE idempotency_key = v_key;
    RETURN v_pid;
  END IF;
  ...
  VALUES (v_me, -v_item.price_coins, 'shop', 'shop', v_key,
    ...
    v_new_balance);
  UPDATE typing_game.profiles SET coin_balance = v_new_balance
  WHERE id = v_me;

  IF p_clan IS NULL THEN
    INSERT INTO typing_game.shop_inventory
    ...
    ON CONFLICT (user_id, item_id) DO UPDATE SET
      ...
      expires_at = CASE WHEN v_item.expiry_days IS NULL THEN NULL
        ELSE now() + (v_item.expiry_days || ' days')::interval END;
  ELSE
    INSERT INTO typing_game.clan_inventory
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_equip_item(
...
BEGIN
  ...
  IF NOT v_item.equippable THEN
    RAISE EXCEPTION 'MALFORMED';
  END IF;
  v_slot := typing_game.shop_equip_slot(v_item.category);
  IF p_clan IS NOT NULL THEN
    IF v_item.item_type <> 'clan_cosmetic' THEN
      RAISE EXCEPTION 'MALFORMED';
    END IF;
    IF NOT typing_game.fn_is_clan_staff(p_clan, v_me) THEN
      RAISE EXCEPTION 'FORBIDDEN';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM typing_game.clan_inventory
                   WHERE clan_id = p_clan AND item_id = p_item
                     AND quantity > 0) THEN
      RAISE EXCEPTION 'NOT_FOUND';
    END IF;
    IF p_equip THEN
      UPDATE typing_game.clan_inventory SET equipped = false
      WHERE clan_id = p_clan AND equipped
        AND item_id IN (SELECT id FROM typing_game.shop_items
                        WHERE typing_game.shop_equip_slot(category) = v_slot);
      UPDATE typing_game.clan_inventory SET equipped = true
      WHERE clan_id = p_clan AND item_id = p_item;
    ELSE
      UPDATE typing_game.clan_inventory SET equipped = false
      WHERE clan_id = p_clan AND item_id = p_item;
  ...
  ELSE
    IF v_item.item_type = 'clan_cosmetic' THEN
      RAISE EXCEPTION 'MALFORMED';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM typing_game.shop_inventory
                   WHERE user_id = v_me AND item_id = p_item
                     AND quantity > 0
    ...
    END IF;
    IF p_equip THEN
      UPDATE typing_game.shop_inventory SET equipped = false
      WHERE user_id = v_me AND equipped
        AND item_id IN (SELECT id FROM typing_game.shop_items
                        WHERE typing_game.shop_equip_slot(category) = v_slot);
      UPDATE typing_game.shop_inventory SET equipped = true
      WHERE user_id = v_me AND item_id = p_item;
    ELSE
      UPDATE typing_game.shop_inventory SET equipped = false
      WHERE user_id = v_me AND item_id = p_item;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_use_consumable(
...
BEGIN
  ...
  IF NOT v_item.consumable OR v_item.effect IS NULL THEN
    RAISE EXCEPTION 'MALFORMED';
  END IF;
  UPDATE typing_game.shop_inventory SET quantity = quantity - 1
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  INSERT INTO typing_game.item_usage (user_id, item_id, quantity_used, context)
  ...
  INSERT INTO typing_game.utility_credits (user_id, effect, balance)
...
CREATE OR REPLACE FUNCTION typing_game.fn_shop_sweep()
RETURNS int
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  v_n int := 0;
BEGIN
  UPDATE typing_game.shop_inventory SET quantity = 0, equipped = false
...
REVOKE ALL ON FUNCTION typing_game.fn_require_shop_admin() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_require_shop_admin() TO authenticated;
REVOKE ALL ON FUNCTION typing_game.shop_equip_slot(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.shop_equip_slot(text) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_create_shop_item(jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_create_shop_item(jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_update_shop_item(uuid, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_update_shop_item(uuid, jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_set_item_active(uuid, boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_set_item_active(uuid, boolean) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_purchase_item(uuid, uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_purchase_item(uuid, uuid, text) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_equip_item(uuid, boolean, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_equip_item(uuid, boolean, uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_use_consumable(uuid, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_use_consumable(uuid, jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_shop_sweep() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_shop_sweep() TO authenticated;
