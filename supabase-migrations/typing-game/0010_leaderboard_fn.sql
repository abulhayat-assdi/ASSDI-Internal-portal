...
CREATE OR REPLACE FUNCTION typing_game.fn_batch_leaderboard(
  p_batch_id uuid,
...
BEGIN
  ...
  IF NOT (
    typing_game.is_batch_member(p_batch_id)
    OR typing_game.is_teacher_of_batch(p_batch_id)
    OR typing_game.is_org_admin(typing_game.batch_organization_id(p_batch_id))
  ) THEN
  ...
  ranked AS (
    SELECT
      ...
      COALESCE(
        (SELECT jsonb_agg(badge_id ORDER BY awarded_at DESC)
         FROM (SELECT ba.badge_id, ba.awarded_at FROM typing_game.badge_awards ba
    ...
    LEFT JOIN typing_game.streaks s ON s.user_id = m.user_id
  )
  SELECT
    ROW_NUMBER() OVER (
...
REVOKE ALL ON FUNCTION
  typing_game.fn_batch_leaderboard(uuid, text, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION
  typing_game.fn_batch_leaderboard(uuid, text, int) TO authenticated;
