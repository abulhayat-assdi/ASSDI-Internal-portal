...
CREATE OR REPLACE FUNCTION typing_game.fn_register_with_batch(
  p_join_code text,
  p_roll_number text,
  p_full_name text,
  p_skill_track typing_game.skill_track
)
RETURNS uuid
...
BEGIN
  ...
  INSERT INTO typing_game.user_roles (user_id, role)
  VALUES (v_user_id, 'student')
  ON CONFLICT DO NOTHING;

  INSERT INTO typing_game.batch_members (batch_id, user_id, roll_number, skill_track, is_active)
...
REVOKE ALL ON FUNCTION typing_game.fn_register_with_batch(text, text, text, typing_game.skill_track)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_register_with_batch(text, text, text, typing_game.skill_track)
  TO authenticated;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.trg_batch_members_audit()
RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  ...
  v_org_id uuid := typing_game.batch_organization_id(v_batch_id);
BEGIN
...
DROP TRIGGER IF EXISTS trg_batch_members_audit_ins ON typing_game.batch_members;
DROP TRIGGER IF EXISTS trg_batch_members_audit_upd ON typing_game.batch_members;

CREATE TRIGGER trg_batch_members_audit_ins
  AFTER INSERT ON typing_game.batch_members
  FOR EACH ROW EXECUTE FUNCTION typing_game.trg_batch_members_audit('batch_member.created');

CREATE TRIGGER trg_batch_members_audit_upd
  AFTER UPDATE OF roll_number, is_active, batch_id ON typing_game.batch_members
  FOR EACH ROW EXECUTE FUNCTION typing_game.trg_batch_members_audit('batch_member.updated');
