...
CREATE OR REPLACE FUNCTION typing_game.fn_finalize_competition(p_competition uuid)
RETURNS jsonb
...
DECLARE
  v_me uuid := auth.uid();
  v_comp typing_game.competitions%ROWTYPE;
  v_scoring jsonb;
  v_metric text;
  v_wpm_cap numeric;
  v_wpm_w numeric;
...
BEGIN
  ...
  END IF;
  IF NOT typing_game.fn_can_manage_competition(p_competition) THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  SELECT * INTO v_comp FROM typing_game.competitions WHERE id = p_competition;
  IF NOT FOUND THEN
  ...
  IF v_reward ? 'badge_first'
     AND NOT EXISTS (
       SELECT 1 FROM typing_game.badges
  ...
  FOR v_entry IN
    ...
    FROM typing_game.competition_entries e
      ...
      CASE
        WHEN v_comp.attempt_policy = 'AVERAGE_TOP_3' THEN (
          SELECT avg(x.v) FROM (
            ...
            FROM typing_game.competition_attempts ca
            JOIN typing_game.game_attempts ga ON ga.id = ca.attempt_id
            JOIN typing_game.attempt_results r ON r.attempt_id = ga.id
  ...
  v_sql := format(
    'INSERT INTO typing_game.competition_results
     ...
     SELECT %L, ''participant'', user_id,
       ...
       value, accuracy, wpm,
       (SELECT count(*) FROM typing_game.competition_attempts WHERE entry_id = w.entry_id),
  ...
  IF jsonb_array_length(COALESCE(v_comp.eligibility -> 'batches', '[]'::jsonb)) > 1 THEN
    ...
    FOR v_ranked IN
      ...
      FROM typing_game.competition_results
      WHERE competition_id = p_competition AND scope = 'batch'
    LOOP
      UPDATE typing_game.competition_results
  ...
  END IF;
    ...
    FOR v_ranked IN
    SELECT ref_id, rank FROM typing_game.competition_results
  ...
  LOOP
    ...
    IF v_xp > 0 THEN
      ...
      FROM typing_game.profiles WHERE id = v_ranked.ref_id
    ...
    IF v_coins > 0 THEN
      ...
      WHERE id = v_ranked.ref_id
        AND EXISTS (
          ...
          WHERE user_id = v_ranked.ref_id AND reference_id = v_key);
    END IF;
    INSERT INTO typing_game.competition_reward_events (key, competition_id, user_id)
    VALUES (v_key, p_competition, v_ranked.ref_id)
    ON CONFLICT (key) DO NOTHING;
    IF v_ranked.rank = 1 AND v_reward ? 'badge_first' THEN
      INSERT INTO typing_game.badge_awards (user_id, badge_id)
      VALUES (v_ranked.ref_id, v_reward ->> 'badge_first')
      ON CONFLICT (user_id, badge_id) DO NOTHING;
    END IF;
  END LOOP;

  UPDATE typing_game.competitions SET status = 'finalized' WHERE id = p_competition;
  INSERT INTO typing_game.competition_state_events
...
CREATE OR REPLACE FUNCTION typing_game.comp_metric_value(
...
CREATE OR REPLACE FUNCTION typing_game.comp_batch_aggregate(
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_record_adjustment(
  p_competition uuid,
  p_scope text,
  p_ref uuid,
  p_patch jsonb,
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM typing_game.competitions
  ...
  END IF;
  SELECT to_jsonb(r) INTO v_old FROM typing_game.competition_results r
  WHERE r.competition_id = p_competition AND r.scope = p_scope AND r.ref_id = p_ref;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;

  UPDATE typing_game.competition_results SET
  ...
  INSERT INTO typing_game.competition_adjustments
    (competition_id, scope, ref_id, old_values, new_values, reason, actor_user_id)
  VALUES (
    p_competition, p_scope, p_ref, v_old,
    (SELECT to_jsonb(r) FROM typing_game.competition_results r
     WHERE r.competition_id = p_competition AND r.scope = p_scope AND r.ref_id = p_ref),
    p_reason, v_me);
END;
$$;

REVOKE ALL ON FUNCTION typing_game.fn_create_competition(text, text, text, typing_game.competition_type, text, uuid[], uuid[], text[], int, numeric, numeric, text[], jsonb, jsonb, text[], text, int, text, int, jsonb, timestamptz, timestamptz, timestamptz, timestamptz) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_create_competition(text, text, text, typing_game.competition_type, text, uuid[], uuid[], text[], int, numeric, numeric, text[], jsonb, jsonb, text[], text, int, text, int, jsonb, timestamptz, timestamptz, timestamptz, timestamptz) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_update_competition_draft(uuid, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_update_competition_draft(uuid, jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_transition_competition(uuid, typing_game.competition_status) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_transition_competition(uuid, typing_game.competition_status) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_register_entry(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_register_entry(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_attach_attempt(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_attach_attempt(uuid, uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_finalize_competition(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_finalize_competition(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_record_adjustment(uuid, text, uuid, jsonb, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_record_adjustment(uuid, text, uuid, jsonb, text) TO authenticated;
