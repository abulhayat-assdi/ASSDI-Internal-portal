...
CREATE OR REPLACE FUNCTION typing_game.fn_check_unlock(
  p_rule jsonb,
  p_level int,
  p_xp int,
  p_acc numeric,
...
BEGIN
  IF v_op IN ('and', 'or') THEN
    FOR v_sub IN SELECT * FROM jsonb_array_elements(p_rule -> 'rules') LOOP
      IF v_op = 'and' AND NOT typing_game.fn_check_unlock(
        v_sub, p_level, p_xp, p_acc, p_wpm, p_games, p_badges) THEN
        RETURN false;
      END IF;
      IF v_op = 'or' AND typing_game.fn_check_unlock(
        v_sub, p_level, p_xp, p_acc, p_wpm, p_games, p_badges) THEN
        RETURN true;
      END IF;
    END LOOP;
...
REVOKE ALL ON FUNCTION
  typing_game.fn_check_unlock(jsonb, int, int, numeric, numeric, text[], text[])
  FROM PUBLIC, anon;
...
-- Main pipeline.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_process_progression(p_attempt_id uuid)
RETURNS jsonb
...
DECLARE
  ...
  v_prof typing_game.profiles%ROWTYPE;
  v_attempt typing_game.game_attempts%ROWTYPE;
  v_result typing_game.attempt_results%ROWTYPE;
  v_profile typing_game.reward_profiles%ROWTYPE;
  v_key text;
  v_is_first boolean;
  v_old_best numeric;
  v_is_pb boolean;
  ...
  v_streak_cur int := 0;
  v_streak_best int := 0;
  v_streak typing_game.streaks%ROWTYPE;
...
BEGIN
  ...
  SELECT * INTO v_prof FROM typing_game.profiles WHERE id = v_user_id FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'PROFILE_NOT_FOUND';
  END IF;
  SELECT * INTO v_attempt FROM typing_game.game_attempts WHERE id = p_attempt_id FOR UPDATE;
  IF NOT FOUND THEN
  ...
  END IF;
  SELECT * INTO v_result FROM typing_game.attempt_results WHERE attempt_id = p_attempt_id;
  IF NOT FOUND THEN
  ...
  SELECT * INTO v_profile FROM typing_game.reward_profiles WHERE id = 'default';

  SELECT NOT EXISTS (
    SELECT 1 FROM typing_game.attempt_results r
    JOIN typing_game.game_attempts a ON a.id = r.attempt_id
  ...
  FROM typing_game.attempt_results r
  JOIN typing_game.game_attempts a ON a.id = r.attempt_id
  WHERE a.user_id = v_user_id AND a.game_id = v_attempt.game_id
    AND a.id <> p_attempt_id AND r.is_valid;
  ...
  SELECT max(level) INTO v_level FROM typing_game.levels WHERE required_xp <= v_new_total;
  ...
  SELECT * INTO v_streak FROM typing_game.streaks WHERE user_id = v_user_id FOR UPDATE;
  v_has_streak := FOUND;
  IF NOT v_has_streak THEN
    INSERT INTO typing_game.streaks
  ...
  ELSIF v_streak.last_active_date = v_event_date THEN
    UPDATE typing_game.streaks SET timezone = v_tz, updated_at = now()
    WHERE user_id = v_user_id;
  ELSIF v_streak.last_active_date = v_event_date - 1 THEN
    UPDATE typing_game.streaks
  ...
  INSERT INTO typing_game.streak_events (user_id, activity_date, attempt_id)
  VALUES (v_user_id, v_event_date, p_attempt_id)
  ON CONFLICT (user_id, activity_date) DO NOTHING;
  UPDATE typing_game.streaks SET active_days = (
    SELECT count(*) FROM typing_game.streak_events WHERE user_id = v_user_id
  ) WHERE user_id = v_user_id;
  SELECT current_count, best_count INTO v_streak_cur, v_streak_best
  FROM typing_game.streaks WHERE user_id = v_user_id;
  ...
  -- -- Badges ----------------------------------------------------------------
  FOR v_badge IN SELECT * FROM typing_game.badges WHERE is_active LOOP
    v_has := CASE v_badge.criteria ->> 'kind'
      ...
      WHEN 'streak_days' THEN (
        SELECT best_count FROM typing_game.streaks WHERE user_id = v_user_id
    ...
    END;
    IF v_has THEN
      INSERT INTO typing_game.badge_awards (user_id, badge_id, attempt_id)
  ...
  -- -- Achievements ------------------------------------------------------------
  FOR v_ach IN SELECT * FROM typing_game.achievements WHERE is_active LOOP
    v_cand := CASE v_ach.metric
      ...
      WHEN 'streak_best' THEN (
        SELECT best_count::numeric FROM typing_game.streaks WHERE user_id = v_user_id
      )
      ELSE -1
    END;
    IF v_cand >= v_ach.threshold THEN
      INSERT INTO typing_game.achievement_awards (user_id, achievement_id, value)
  ...
  -- best_score
  INSERT INTO typing_game.personal_records (user_id, game_id, metric, value, attempt_id)
  ...
  ON CONFLICT (user_id, game_id, metric) DO UPDATE
    SET value = EXCLUDED.value, attempt_id = EXCLUDED.attempt_id,
        recorded_at = now()
    WHERE EXCLUDED.value > typing_game.personal_records.value
  ...
  INTO v_badges
  FROM typing_game.badge_awards WHERE user_id = v_user_id;

  FOR v_game IN SELECT slug, unlock_rule FROM typing_game.games WHERE is_active LOOP
    IF typing_game.fn_check_unlock(
...
REVOKE ALL ON FUNCTION typing_game.fn_process_progression(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_process_progression(uuid) TO authenticated;
