...
CREATE TABLE IF NOT EXISTS typing_game.worlds (
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.scoring_profiles (
  id text PRIMARY KEY CHECK (char_length(id) BETWEEN 1 AND 80),
...
INSERT INTO typing_game.scoring_profiles
...
CREATE TABLE IF NOT EXISTS typing_game.difficulty_profiles (
  id text PRIMARY KEY CHECK (char_length(id) BETWEEN 1 AND 80),
...
INSERT INTO typing_game.difficulty_profiles
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.prompt_sets (
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.games (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  slug text NOT NULL UNIQUE CHECK (char_length(slug) BETWEEN 1 AND 80),
  world_id text NOT NULL REFERENCES typing_game.worlds (id) ON DELETE RESTRICT,
  category text NOT NULL,
  mechanic text NOT NULL,
  mode text NOT NULL,
  difficulty text NOT NULL,
  skill_bands text[] NOT NULL DEFAULT '{}',
  prompt_set_ref text NOT NULL REFERENCES typing_game.prompt_sets (ref) ON DELETE RESTRICT,
  prompt_units int NOT NULL DEFAULT 10 CHECK (prompt_units > 0),
  input jsonb NOT NULL DEFAULT '{}'::jsonb,
  timing jsonb NOT NULL DEFAULT '{}'::jsonb,
  scoring_profile_id text NOT NULL REFERENCES typing_game.scoring_profiles (id) ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS games_world_id_idx ON typing_game.games (world_id);
CREATE INDEX IF NOT EXISTS games_active_idx ON typing_game.games (is_active) WHERE is_active;

CREATE TABLE IF NOT EXISTS typing_game.game_versions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS game_versions_game_id_idx ON typing_game.game_versions (game_id);
...
-- ---------------------------------------------------------------------------
ALTER TABLE typing_game.worlds ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.scoring_profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.difficulty_profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.prompt_sets ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.games ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.game_versions ENABLE ROW LEVEL SECURITY;

CREATE POLICY worlds_select_public ON typing_game.worlds
  FOR SELECT TO anon, authenticated USING (true);

CREATE POLICY scoring_profiles_select_public ON typing_game.scoring_profiles
  FOR SELECT TO anon, authenticated USING (true);

CREATE POLICY difficulty_profiles_select_public ON typing_game.difficulty_profiles
  FOR SELECT TO anon, authenticated USING (true);

CREATE POLICY games_select_active ON typing_game.games
  FOR SELECT TO anon, authenticated USING (is_active);

CREATE POLICY game_versions_select_active_game ON typing_game.game_versions
