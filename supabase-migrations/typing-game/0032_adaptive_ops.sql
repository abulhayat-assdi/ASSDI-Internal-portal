...
ALTER TABLE typing_game.adaptive_attempt_samples
...
CREATE OR REPLACE FUNCTION typing_game.adaptive_classify_pair(
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_record_adaptive_attempt(
...
BEGIN
  ...
  END IF;
  SELECT * INTO v_att FROM typing_game.game_attempts WHERE id = p_attempt;
  ...
  END IF;
  SELECT * INTO v_res FROM typing_game.attempt_results WHERE attempt_id = p_attempt;
  IF NOT FOUND OR NOT v_res.is_valid THEN
    RAISE EXCEPTION 'INVALID_STATE';
  END IF;
  SELECT slug, mechanic, prompt_set_ref INTO v_game
  FROM typing_game.games WHERE id = v_att.game_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  ...
  INSERT INTO typing_game.adaptive_attempt_samples
  ...
  SELECT v_me, p_attempt, v_game.slug,
         ...
         COALESCE((SELECT kind FROM typing_game.prompt_sets
  ...
  INSERT INTO typing_game.adaptive_key_stats
  ...
  INSERT INTO typing_game.learner_skill_profiles
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_adaptive_recompute(p_user uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  ...
  v_priority numeric;
BEGIN
  INSERT INTO typing_game.learner_skill_profiles (user_id)
  VALUES (p_user)
  ON CONFLICT (user_id) DO NOTHING;

  -- Dimensions from the last 50 samples (conditional means + counts).
  INSERT INTO typing_game.learner_skill_dimensions
  ...
  FROM (
    ...
    FROM (SELECT accuracy FROM typing_game.adaptive_attempt_samples
          WHERE user_id = p_user ORDER BY submitted_at DESC LIMIT 50) AS s
    UNION ALL
    SELECT 'wpm', avg(s.wpm), count(*)
    FROM (SELECT wpm FROM typing_game.adaptive_attempt_samples
          WHERE user_id = p_user ORDER BY submitted_at DESC LIMIT 50) AS s
    UNION ALL
    SELECT 'completion', avg(s.completion), count(*)
    FROM (SELECT completion FROM typing_game.adaptive_attempt_samples
          WHERE user_id = p_user ORDER BY submitted_at DESC LIMIT 50) AS s
    UNION ALL
    SELECT 'error_rate',
      CASE WHEN sum(s.prompt_len) > 0
           ...
           ELSE 0 END,
      count(*)
    FROM (SELECT error_strokes, prompt_len FROM typing_game.adaptive_attempt_samples
          WHERE user_id = p_user ORDER BY submitted_at DESC LIMIT 50) AS s
    UNION ALL
    SELECT 'consistency',
  ...
  SELECT value INTO v_acc FROM typing_game.learner_skill_dimensions
  WHERE user_id = p_user AND dimension = 'accuracy';
  SELECT value INTO v_wpm FROM typing_game.learner_skill_dimensions
  WHERE user_id = p_user AND dimension = 'wpm';
  SELECT evidence INTO v_n FROM typing_game.learner_skill_dimensions
  WHERE user_id = p_user AND dimension = 'accuracy';

  -- Fingers re-derive from key evidence (same QWERTY map as TS).
  INSERT INTO typing_game.adaptive_finger_stats
    (user_id, finger, exposures, errors)
  SELECT p_user, typing_game.adaptive_finger_for(key_char),
         sum(exposures), sum(errors)
  FROM typing_game.adaptive_key_stats
  WHERE user_id = p_user
    AND typing_game.adaptive_finger_for(key_char) IS NOT NULL
  GROUP BY typing_game.adaptive_finger_for(key_char)
  ...
  INSERT INTO typing_game.adaptive_skill_trends
  ...
  FROM (
    SELECT 'accuracy' AS metric,
      CASE WHEN (SELECT count(*) FROM typing_game.adaptive_attempt_samples
      ...
      (SELECT count(*) FROM typing_game.adaptive_attempt_samples WHERE user_id = p_user) AS n
    UNION ALL
    SELECT 'wpm',
      CASE WHEN (SELECT count(*) FROM typing_game.adaptive_attempt_samples
                 WHERE user_id = p_user) >= 5 THEN
      ...
      END,
      2::numeric, 1::numeric,
      (SELECT count(*) FROM typing_game.adaptive_attempt_samples WHERE user_id = p_user)
    UNION ALL
    SELECT 'error_rate',
      CASE WHEN (SELECT count(*) FROM typing_game.adaptive_attempt_samples
                 WHERE user_id = p_user) >= 5 THEN
        (SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY e)
         FROM (SELECT CASE WHEN prompt_len > 0
               ...
               FROM typing_game.adaptive_attempt_samples
               WHERE user_id = p_user ORDER BY submitted_at DESC LIMIT 10) AS r)
      END,
      CASE WHEN (SELECT count(*) FROM typing_game.adaptive_attempt_samples
                 WHERE user_id = p_user) >= 15 THEN
  ...
  DELETE FROM typing_game.adaptive_weaknesses WHERE user_id = p_user;
  INSERT INTO typing_game.adaptive_weaknesses
  ...
  FROM (
    SELECT 'key' AS target_type, ks.key_char AS target,
      CASE WHEN ks.key_char ~ '^[0-9]$' THEN 'numbers'
           ...
           WHEN EXISTS (SELECT 1 FROM typing_game.adaptive_error_pairs ep
      ...
      COALESCE((SELECT trend FROM typing_game.adaptive_skill_trends
                WHERE user_id = p_user AND metric = 'accuracy'),
               'insufficient') AS trend
    FROM typing_game.adaptive_key_stats ks
    ...
    SELECT 'finger', fs.finger, 'words',
      ...
      COALESCE((SELECT trend FROM typing_game.adaptive_skill_trends
                WHERE user_id = p_user AND metric = 'accuracy'),
               'insufficient')
    FROM typing_game.adaptive_finger_stats fs
    ...
    SELECT 'dimension', 'accuracy', 'words',
      ...
      COALESCE((SELECT trend FROM typing_game.adaptive_skill_trends
                WHERE user_id = p_user AND metric = 'accuracy'),
               'insufficient')
    FROM typing_game.learner_skill_dimensions d
  ...
  ELSE
    v_band := 'beginner';
  END IF;
  UPDATE typing_game.learner_skill_profiles
  ...
  FOR v_cand IN
    ...
    FROM typing_game.adaptive_attempt_samples s
    WHERE s.user_id = p_user
      AND s.submitted_at > now() - interval '30 days'
    LIMIT 20
  LOOP
    SELECT band INTO v_game_band FROM typing_game.adaptive_game_difficulty
    WHERE user_id = p_user AND game_slug = v_cand.g;
    IF NOT FOUND OR v_game_band IS NULL THEN
      v_game_band := 'beginner';
    END IF;
    INSERT INTO typing_game.adaptive_game_difficulty
    ...
    FROM (
      ...
      FROM (SELECT accuracy, wpm FROM typing_game.adaptive_attempt_samples
            WHERE user_id = p_user AND game_slug = v_cand.g
            ORDER BY submitted_at DESC LIMIT 3) AS f,
           (SELECT target_accuracy, target_wpm FROM typing_game.difficulty_profiles
    ...
    LATERAL (
      ...
      FROM typing_game.difficulty_profiles dp2 WHERE dp2.id = nb2.band
    ) AS nb
  ...
  UPDATE typing_game.adaptive_recommendations SET status = 'superseded'
  WHERE user_id = p_user AND status = 'active';
  FOR v_sig IN
    SELECT target_type, target, prompt_kind, score, confidence, trend
    FROM typing_game.adaptive_weaknesses
    WHERE user_id = p_user
    ORDER BY score DESC
    LIMIT 5
  LOOP
    ...
    FROM typing_game.games gs
    JOIN typing_game.prompt_sets ps ON ps.ref = gs.prompt_set_ref
    WHERE gs.is_active AND ps.kind = v_sig.prompt_kind
      AND (
        EXISTS (SELECT 1 FROM typing_game.game_unlocks u
                WHERE u.user_id = p_user AND u.game_slug = gs.slug
                  AND u.unlocked)
        OR COALESCE(gs.unlock_rule ->> 'type', 'open') = 'open')
      AND NOT EXISTS (SELECT 1 FROM typing_game.adaptive_exposure e
    ...
    FROM typing_game.daily_mission_assignments a
    JOIN typing_game.missions m ON m.id = a.mission_id
    ...
    ELSE
      SELECT m.id INTO v_mid
      FROM typing_game.weekly_challenges a
      JOIN typing_game.missions m ON m.id = a.mission_id
    ...
    FROM typing_game.adaptive_exposure e
    ...
    FROM typing_game.games gs WHERE gs.slug = v_g;
    v_priority :=
    ...
    v_rank := v_rank + 1;
    INSERT INTO typing_game.adaptive_recommendations
    ...
    v_recs := v_recs + 1;
    INSERT INTO typing_game.adaptive_exposure (user_id, game_slug, shows_in_window)
    VALUES (p_user, v_g, 1)
    ON CONFLICT (user_id, game_slug) DO UPDATE SET
...
CREATE OR REPLACE FUNCTION typing_game.fn_refresh_adaptive_profile()
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  IF v_me IS NULL THEN
    RAISE EXCEPTION 'NOT_AUTHENTICATED';
  END IF;
  RETURN typing_game.fn_adaptive_recompute(v_me);
END;
$$;

-- Cheap cached summary for dashboards (never recomputes).
CREATE OR REPLACE FUNCTION typing_game.fn_get_adaptive_summary()
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'profile', (SELECT to_jsonb(p) FROM typing_game.learner_skill_profiles p
                WHERE p.user_id = v_me),
    'dimensions', (SELECT COALESCE(jsonb_agg(
      jsonb_build_object('dimension', dimension, 'value', value,
                         'evidence', evidence)
      ORDER BY dimension), '[]'::jsonb)
      FROM typing_game.learner_skill_dimensions WHERE user_id = v_me),
    'trends', (SELECT COALESCE(jsonb_agg(
      ...
      FROM typing_game.adaptive_skill_trends WHERE user_id = v_me),
    'weaknesses', (SELECT COALESCE(jsonb_agg(
      ...
      FROM typing_game.adaptive_weaknesses WHERE user_id = v_me),
    'recommendations', (SELECT COALESCE(jsonb_agg(
      ...
      ORDER BY rank), '[]'::jsonb)
      FROM typing_game.adaptive_recommendations
    ...
    'difficulty', (SELECT COALESCE(jsonb_agg(
      ...
      FROM typing_game.adaptive_game_difficulty WHERE user_id = v_me));
END;
$$;

-- Feedback: students report what happened (never scores).
CREATE OR REPLACE FUNCTION typing_game.fn_adaptive_feedback(
...
BEGIN
  ...
  IF p_event NOT IN ('shown', 'started', 'completed', 'abandoned', 'skipped') THEN
    RAISE EXCEPTION 'MALFORMED';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM typing_game.adaptive_recommendations
                 WHERE id = p_recommendation AND user_id = v_me) THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  INSERT INTO typing_game.adaptive_recommendation_events
    (recommendation_id, user_id, event)
  VALUES (p_recommendation, v_me, p_event);
  IF p_event = 'started' THEN
    UPDATE typing_game.adaptive_recommendations SET status = 'accepted'
    WHERE id = p_recommendation AND status = 'active';
  ELSIF p_event = 'completed' THEN
    UPDATE typing_game.adaptive_recommendations SET status = 'completed'
    WHERE id = p_recommendation AND status IN ('active', 'accepted');
  ELSIF p_event = 'skipped' THEN
    UPDATE typing_game.adaptive_recommendations SET status = 'dismissed'
    WHERE id = p_recommendation AND status = 'active';
  END IF;
END;
$$;
...
CREATE OR REPLACE FUNCTION typing_game.fn_adaptive_batch_summary(p_batch uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
BEGIN
  ...
  -- caller's admin role is scoped to this batch's own organization, which
  -- made the is_org_admin(...) check below dead code for any admin.
  -- is_org_admin(batch_organization_id(p_batch)) already covers super_admin
  -- internally, but the explicit is_super_admin() check is kept so the
  -- fallthrough is obvious for a batch with a since-deleted organization.
  IF NOT typing_game.is_teacher_of_batch(p_batch)
     AND NOT typing_game.is_super_admin()
     AND NOT typing_game.is_org_admin(
       typing_game.batch_organization_id(p_batch)) THEN
  ...
  RETURN jsonb_build_object(
    ...
    'attention', (SELECT COALESCE(jsonb_agg(x ORDER BY x.user_id), '[]'::jsonb)
      FROM (
        SELECT bm.user_id,
          EXISTS (SELECT 1 FROM typing_game.adaptive_skill_trends t
          ...
          (SELECT count(*) FROM typing_game.adaptive_key_stats ks
    ...
    'averages', (SELECT jsonb_build_object(
      ...
      FROM typing_game.learner_skill_dimensions d
      JOIN typing_game.batch_members bm ON bm.user_id = d.user_id
      WHERE bm.batch_id = p_batch AND bm.is_active),
    ...
    'bands', (SELECT COALESCE(jsonb_agg(x), '[]'::jsonb)
      FROM (
        ...
        FROM typing_game.learner_skill_profiles p
        JOIN typing_game.batch_members bm ON bm.user_id = p.user_id
        WHERE bm.batch_id = p_batch AND bm.is_active
...
CREATE OR REPLACE FUNCTION typing_game.fn_adaptive_global_summary()
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
BEGIN
  -- Platform-wide aggregate over every organization's learners: this is a
  -- true super-admin analytics tool, not an org-admin one. fn_is_mission_admin()
  -- must not be used here — it cannot distinguish "admin of one organization"
  -- from platform-wide access, and this function has no organization
  -- parameter to scope by.
  IF NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  RETURN jsonb_build_object(
    'learners', (SELECT count(*) FROM typing_game.learner_skill_profiles),
    'avg_accuracy', (SELECT avg(value) FROM typing_game.learner_skill_dimensions
                     WHERE dimension = 'accuracy' AND evidence >= 3),
    'avg_wpm', (SELECT avg(value) FROM typing_game.learner_skill_dimensions
    ...
    'funnel', (SELECT jsonb_build_object(
      ...
      FROM typing_game.adaptive_recommendation_events
...
CREATE OR REPLACE FUNCTION typing_game.fn_adaptive_sweep()
RETURNS int
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  FOR r IN
    SELECT user_id FROM typing_game.learner_skill_profiles
  ...
  LOOP
    BEGIN
      PERFORM typing_game.fn_adaptive_recompute(r.user_id);
      v_n := v_n + 1;
...
REVOKE ALL ON FUNCTION typing_game.adaptive_finger_for(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.adaptive_finger_for(text) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.adaptive_classify_pair(text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.adaptive_classify_pair(text, text) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_record_adaptive_attempt(uuid, jsonb, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_record_adaptive_attempt(uuid, jsonb, jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_adaptive_recompute(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_adaptive_recompute(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_refresh_adaptive_profile() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_refresh_adaptive_profile() TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_get_adaptive_summary() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_get_adaptive_summary() TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_adaptive_feedback(uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_adaptive_feedback(uuid, text) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_adaptive_batch_summary(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_adaptive_batch_summary(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_adaptive_global_summary() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_adaptive_global_summary() TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_adaptive_sweep() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_adaptive_sweep() TO authenticated;
