...
CREATE TABLE IF NOT EXISTS typing_game.shop_categories (
...
INSERT INTO typing_game.shop_categories (slug, name_en, name_bn, sort_order)
VALUES
...
CREATE TABLE IF NOT EXISTS typing_game.shop_items (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  slug text NOT NULL UNIQUE CHECK (char_length(slug) BETWEEN 1 AND 80),
  name text NOT NULL CHECK (char_length(name) BETWEEN 1 AND 160),
  description text NOT NULL DEFAULT '',
  category text NOT NULL REFERENCES typing_game.shop_categories (slug)
    ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS typing_game.shop_item_versions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  item_id uuid NOT NULL REFERENCES typing_game.shop_items (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS typing_game.shop_inventory (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  item_id uuid NOT NULL REFERENCES typing_game.shop_items (id) ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS shop_inventory_user_idx
  ON typing_game.shop_inventory (user_id);

-- Clan inventory: owned by the clan, never mixed with personal rows.
CREATE TABLE IF NOT EXISTS typing_game.clan_inventory (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  clan_id uuid NOT NULL REFERENCES typing_game.clans (id) ON DELETE CASCADE,
  item_id uuid NOT NULL REFERENCES typing_game.shop_items (id) ON DELETE RESTRICT,
  quantity int NOT NULL DEFAULT 0 CHECK (quantity >= 0),
  equipped boolean NOT NULL DEFAULT false,
  granted_by uuid REFERENCES typing_game.profiles (id) ON DELETE SET NULL,
  granted_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (clan_id, item_id)
);

-- Immutable purchase ledger (append-only; guarded by trigger).
CREATE TABLE IF NOT EXISTS typing_game.purchase_records (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  idempotency_key text NOT NULL UNIQUE,
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  clan_id uuid REFERENCES typing_game.clans (id) ON DELETE SET NULL,
  item_id uuid NOT NULL REFERENCES typing_game.shop_items (id) ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS purchase_user_idx
  ON typing_game.purchase_records (user_id, created_at DESC);

-- Grant audit (every inventory credit traced to a cause).
CREATE TABLE IF NOT EXISTS typing_game.item_grants (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  purchase_id uuid REFERENCES typing_game.purchase_records (id) ON DELETE SET NULL,
  user_id uuid REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  clan_id uuid REFERENCES typing_game.clans (id) ON DELETE CASCADE,
  item_id uuid NOT NULL REFERENCES typing_game.shop_items (id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS typing_game.item_usage (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  item_id uuid NOT NULL REFERENCES typing_game.shop_items (id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS typing_game.utility_credits (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
...
CREATE OR REPLACE FUNCTION typing_game.fn_guard_purchase_immutable()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS guard_purchase_immutable ON typing_game.purchase_records;
CREATE TRIGGER guard_purchase_immutable
BEFORE UPDATE OR DELETE ON typing_game.purchase_records
FOR EACH ROW EXECUTE FUNCTION typing_game.fn_guard_purchase_immutable();
...
-- ---------------------------------------------------------------------------
ALTER TABLE typing_game.shop_categories ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.shop_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.shop_item_versions ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.shop_inventory ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.clan_inventory ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.purchase_records ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.item_grants ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.item_usage ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.utility_credits ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS shop_categories_select ON typing_game.shop_categories;
DROP POLICY IF EXISTS shop_items_select ON typing_game.shop_items;
DROP POLICY IF EXISTS shop_items_admin ON typing_game.shop_items;
DROP POLICY IF EXISTS shop_versions_admin ON typing_game.shop_item_versions;
DROP POLICY IF EXISTS shop_inventory_own ON typing_game.shop_inventory;
DROP POLICY IF EXISTS clan_inventory_view ON typing_game.clan_inventory;
DROP POLICY IF EXISTS purchase_own ON typing_game.purchase_records;
DROP POLICY IF EXISTS purchase_admin ON typing_game.purchase_records;
DROP POLICY IF EXISTS grants_own ON typing_game.item_grants;
DROP POLICY IF EXISTS usage_own ON typing_game.item_usage;
DROP POLICY IF EXISTS credits_own ON typing_game.utility_credits;

CREATE POLICY shop_categories_select ON typing_game.shop_categories
  FOR SELECT TO authenticated USING (is_active);
CREATE POLICY shop_items_select ON typing_game.shop_items
  FOR SELECT TO authenticated USING (is_active);
CREATE POLICY shop_items_admin ON typing_game.shop_items
  FOR SELECT TO authenticated
  USING (typing_game.fn_is_mission_admin() OR typing_game.is_super_admin());
CREATE POLICY shop_versions_admin ON typing_game.shop_item_versions
  FOR SELECT TO authenticated
  USING (typing_game.fn_is_mission_admin() OR typing_game.is_super_admin());
CREATE POLICY shop_inventory_own ON typing_game.shop_inventory
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY clan_inventory_view ON typing_game.clan_inventory
  FOR SELECT TO authenticated USING (typing_game.fn_is_clan_viewer(clan_id));
CREATE POLICY purchase_own ON typing_game.purchase_records
  FOR SELECT TO authenticated USING (user_id = auth.uid());
-- fn_is_mission_admin() alone cannot gate this: purchase_records is
-- per-student data reachable to an org admin only for their own
-- organization's students (via batch_members -> batches -> courses),
-- which is what is_org_admin_of_user(user_id) checks.
CREATE POLICY purchase_admin ON typing_game.purchase_records
  FOR SELECT TO authenticated
  USING (typing_game.is_org_admin_of_user(user_id));
CREATE POLICY grants_own ON typing_game.item_grants
  FOR SELECT TO authenticated USING (
    user_id = auth.uid()
    OR (clan_id IS NOT NULL AND typing_game.fn_is_clan_viewer(clan_id)));
CREATE POLICY usage_own ON typing_game.item_usage
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY credits_own ON typing_game.utility_credits
  FOR SELECT TO authenticated USING (user_id = auth.uid());
