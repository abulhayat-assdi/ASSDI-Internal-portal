...
DO $$ BEGIN
  CREATE TYPE typing_game.attempt_status AS ENUM (
...
CREATE TABLE IF NOT EXISTS typing_game.game_attempts (
  ...
  game_version_id uuid NOT NULL REFERENCES typing_game.game_versions (id) ON DELETE RESTRICT,
  ...
  status typing_game.attempt_status NOT NULL DEFAULT 'started',
...
CREATE INDEX IF NOT EXISTS attempts_user_idx
  ON typing_game.game_attempts (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS attempts_game_status_idx
  ON typing_game.game_attempts (game_id, status);
CREATE INDEX IF NOT EXISTS attempts_expiry_idx
  ON typing_game.game_attempts (status, expires_at)
  WHERE status IN ('created', 'started', 'in_progress', 'submitted');

CREATE TABLE IF NOT EXISTS typing_game.attempt_results (
  attempt_id uuid PRIMARY KEY REFERENCES typing_game.game_attempts (id) ON DELETE CASCADE,
  raw jsonb NOT NULL,
...
ALTER TABLE typing_game.game_attempts ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.attempt_results ENABLE ROW LEVEL SECURITY;

-- Owners read their own attempts + results. Nothing else for API roles.
CREATE POLICY attempts_select_own ON typing_game.game_attempts
  FOR SELECT TO authenticated USING (user_id = auth.uid());

CREATE POLICY results_select_own ON typing_game.attempt_results
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_start_attempt(
  p_game_slug text,
  p_difficulty text,
  p_seed text,
  p_expected text,
...
BEGIN
  ...
  FROM typing_game.games g
  JOIN typing_game.game_versions gv
  ...
  IF (
    SELECT count(*) FROM typing_game.game_attempts
  ...
  INSERT INTO typing_game.game_attempts
...
REVOKE ALL ON FUNCTION
  typing_game.fn_start_attempt(text, text, text, text, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION
  typing_game.fn_start_attempt(text, text, text, text, int) TO authenticated;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_submit_attempt(
  p_attempt_id uuid,
  p_raw jsonb,
  p_score numeric,
  p_accuracy numeric,
...
RETURNS typing_game.attempt_status
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  v_user_id uuid := auth.uid();
  v_row typing_game.game_attempts%ROWTYPE;
  v_final typing_game.attempt_status;
BEGIN
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION 'NOT_AUTHENTICATED';
  END IF;

  SELECT * INTO v_row FROM typing_game.game_attempts
  ...
  v_final := CASE WHEN p_valid THEN 'validated'::typing_game.attempt_status
                 ELSE 'rejected'::typing_game.attempt_status END;

  UPDATE typing_game.game_attempts
  SET status = v_final, submitted_at = now(), finalized_at = now()
  WHERE id = p_attempt_id;

  INSERT INTO typing_game.attempt_results
...
REVOKE ALL ON FUNCTION
  typing_game.fn_submit_attempt(uuid, jsonb, numeric, numeric, numeric, boolean, text)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION
  typing_game.fn_submit_attempt(uuid, jsonb, numeric, numeric, numeric, boolean, text)
  TO authenticated;
