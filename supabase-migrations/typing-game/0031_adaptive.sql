...
CREATE TABLE IF NOT EXISTS typing_game.learner_skill_profiles (
  user_id uuid PRIMARY KEY REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS typing_game.learner_skill_dimensions (
...
CREATE TABLE IF NOT EXISTS typing_game.adaptive_key_stats (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS typing_game.adaptive_finger_stats (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS typing_game.adaptive_error_pairs (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS typing_game.adaptive_attempt_samples (
...
CREATE INDEX IF NOT EXISTS adaptive_samples_user_idx
  ON typing_game.adaptive_attempt_samples (user_id, submitted_at DESC);

-- Cached trends (recent window vs baseline medians).
CREATE TABLE IF NOT EXISTS typing_game.adaptive_skill_trends (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS typing_game.adaptive_weaknesses (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS typing_game.adaptive_recommendations (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  rank int NOT NULL CHECK (rank > 0),
  game_slug text NOT NULL REFERENCES typing_game.games (slug) ON DELETE RESTRICT,
  difficulty text NOT NULL DEFAULT 'beginner',
  mission_id uuid REFERENCES typing_game.missions (id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS adaptive_recs_user_idx
  ON typing_game.adaptive_recommendations (user_id, status, rank);

-- Observed recommendation outcomes (students cannot write scores).
CREATE TABLE IF NOT EXISTS typing_game.adaptive_recommendation_events (
  id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  recommendation_id uuid NOT NULL REFERENCES typing_game.adaptive_recommendations (id)
    ON DELETE CASCADE,
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  event text NOT NULL
    CHECK (event IN (
...
CREATE INDEX IF NOT EXISTS adaptive_rec_events_user_idx
  ON typing_game.adaptive_recommendation_events (user_id, created_at DESC);

-- Recent exposure per game (fatigue/diversity control).
CREATE TABLE IF NOT EXISTS typing_game.adaptive_exposure (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  game_slug text NOT NULL,
...
CREATE TABLE IF NOT EXISTS typing_game.adaptive_game_difficulty (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  game_slug text NOT NULL,
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.adaptive_finger_for(p_key text)
RETURNS text
...
-- ---------------------------------------------------------------------------
ALTER TABLE typing_game.learner_skill_profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.learner_skill_dimensions ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.adaptive_key_stats ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.adaptive_finger_stats ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.adaptive_error_pairs ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.adaptive_attempt_samples ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.adaptive_skill_trends ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.adaptive_weaknesses ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.adaptive_recommendations ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.adaptive_recommendation_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.adaptive_exposure ENABLE ROW LEVEL SECURITY;
ALTER TABLE typing_game.adaptive_game_difficulty ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS adaptive_profile_own ON typing_game.learner_skill_profiles;
DROP POLICY IF EXISTS adaptive_dims_own ON typing_game.learner_skill_dimensions;
DROP POLICY IF EXISTS adaptive_keys_own ON typing_game.adaptive_key_stats;
DROP POLICY IF EXISTS adaptive_fingers_own ON typing_game.adaptive_finger_stats;
DROP POLICY IF EXISTS adaptive_pairs_own ON typing_game.adaptive_error_pairs;
DROP POLICY IF EXISTS adaptive_samples_own ON typing_game.adaptive_attempt_samples;
DROP POLICY IF EXISTS adaptive_trends_own ON typing_game.adaptive_skill_trends;
DROP POLICY IF EXISTS adaptive_weak_own ON typing_game.adaptive_weaknesses;
DROP POLICY IF EXISTS adaptive_recs_own ON typing_game.adaptive_recommendations;
DROP POLICY IF EXISTS adaptive_events_own ON typing_game.adaptive_recommendation_events;
DROP POLICY IF EXISTS adaptive_exposure_own ON typing_game.adaptive_exposure;
DROP POLICY IF EXISTS adaptive_diff_own ON typing_game.adaptive_game_difficulty;

CREATE POLICY adaptive_profile_own ON typing_game.learner_skill_profiles
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY adaptive_dims_own ON typing_game.learner_skill_dimensions
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY adaptive_keys_own ON typing_game.adaptive_key_stats
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY adaptive_fingers_own ON typing_game.adaptive_finger_stats
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY adaptive_pairs_own ON typing_game.adaptive_error_pairs
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY adaptive_samples_own ON typing_game.adaptive_attempt_samples
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY adaptive_trends_own ON typing_game.adaptive_skill_trends
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY adaptive_weak_own ON typing_game.adaptive_weaknesses
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY adaptive_recs_own ON typing_game.adaptive_recommendations
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY adaptive_events_own ON typing_game.adaptive_recommendation_events
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY adaptive_exposure_own ON typing_game.adaptive_exposure
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY adaptive_diff_own ON typing_game.adaptive_game_difficulty
  FOR SELECT TO authenticated USING (user_id = auth.uid());
