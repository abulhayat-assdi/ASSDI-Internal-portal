...
CREATE OR REPLACE FUNCTION typing_game.fn_log_tournament(
  p_tournament uuid, p_from typing_game.tournament_status,
  p_to typing_game.tournament_status, p_actor uuid
)
RETURNS void
LANGUAGE sql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
  INSERT INTO typing_game.tournament_state_events
...
CREATE OR REPLACE FUNCTION typing_game.fn_require_tournament_admin(
...
DECLARE
  v_org uuid;
BEGIN
  -- fn_is_mission_admin() alone cannot gate this: it does not verify the
  -- caller's admin role is scoped to the target tournament's/org's own
  -- organization. Falling through to is_org_admin(v_org) below (which
  -- requires v_org IS NOT NULL) is the correct, org-scoped check and also
  -- keeps platform-wide (organization-less) tournaments super_admin-only.
  IF typing_game.is_super_admin() THEN
    RETURN;
  END IF;
  v_org := p_org;
  IF p_tournament IS NOT NULL THEN
    SELECT organization_id INTO v_org FROM typing_game.tournaments
    WHERE id = p_tournament;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'NOT_FOUND';
    END IF;
  END IF;
  IF v_org IS NOT NULL AND typing_game.is_org_admin(v_org) THEN
    RETURN;
  END IF;
...
CREATE OR REPLACE FUNCTION typing_game.fn_create_tournament(p_def jsonb)
RETURNS uuid
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  PERFORM typing_game.fn_require_tournament_admin(NULL, v_org);
  ...
  IF (p_def ->> 'end_at')::timestamptz IS NOT NULL
    ...
    RAISE EXCEPTION 'MALFORMED';
  END IF;
  INSERT INTO typing_game.tournaments (
  ...
  VALUES (
    ...
    v_format::typing_game.tournament_format,
    v_ptype::typing_game.tournament_participant_type,
    v_org,
  ...
  INSERT INTO typing_game.tournament_versions (tournament_id, version, definition)
  SELECT v_id, version, to_jsonb(t.*) - 'created_at' - 'updated_at'
  FROM typing_game.tournaments t WHERE t.id = v_id;
  RETURN v_id;
...
CREATE OR REPLACE FUNCTION typing_game.fn_update_tournament_draft(
...
AS $$
BEGIN
  PERFORM typing_game.fn_require_tournament_admin(p_tournament);
  IF NOT EXISTS (SELECT 1 FROM typing_game.tournaments
                 WHERE id = p_tournament AND status = 'draft') THEN
    RAISE EXCEPTION 'NOT_DRAFT';
  END IF;
  UPDATE typing_game.tournaments SET
  ...
  FROM typing_game.tournaments t WHERE t.id = p_tournament;
END;
$$;

-- publish: draft -> registration_open. close: registration_open ->
-- registration_closed. cancel: where permitted.
CREATE OR REPLACE FUNCTION typing_game.fn_publish_tournament(p_tournament uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
BEGIN
  PERFORM typing_game.fn_require_tournament_admin(p_tournament);
  UPDATE typing_game.tournaments SET status = 'registration_open',
    updated_at = now()
  WHERE id = p_tournament AND status = 'draft';
  IF NOT FOUND THEN
    RAISE EXCEPTION 'INVALID_STATE';
  END IF;
  PERFORM typing_game.fn_log_tournament(
    p_tournament, 'draft', 'registration_open', auth.uid());
END;
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_close_registration(p_tournament uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
BEGIN
  PERFORM typing_game.fn_require_tournament_admin(p_tournament);
  UPDATE typing_game.tournaments SET status = 'registration_closed',
    updated_at = now()
...
CREATE OR REPLACE FUNCTION typing_game.fn_cancel_tournament(p_tournament uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  v_status typing_game.tournament_status;
BEGIN
  PERFORM typing_game.fn_require_tournament_admin(p_tournament);
  SELECT status INTO v_status FROM typing_game.tournaments WHERE id = p_tournament;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  IF v_status NOT IN ('draft', 'registration_open', 'registration_closed',
    ...
    RAISE EXCEPTION 'INVALID_STATE';
  END IF;
  UPDATE typing_game.tournaments SET status = 'cancelled', updated_at = now()
  WHERE id = p_tournament;
  UPDATE typing_game.tournament_matches SET status = 'cancelled',
    updated_at = now()
  WHERE tournament_id = p_tournament AND status NOT IN ('finalized', 'bye');
  UPDATE typing_game.tournament_rounds SET status = 'cancelled'
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_register_tournament(
...
BEGIN
  IF v_me IS NULL THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  SELECT * INTO v_t FROM typing_game.tournaments WHERE id = p_tournament;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  ...
  ELSE
    IF p_clan IS NULL THEN
      RAISE EXCEPTION 'MALFORMED';
    END IF;
    IF NOT typing_game.fn_is_clan_staff(p_clan, v_me)
       AND NOT typing_game.fn_is_tournament_admin(p_tournament) THEN
      RAISE EXCEPTION 'FORBIDDEN';
    END IF;
    v_pid := p_clan;
    SELECT name INTO v_name FROM typing_game.clans
  ...
  SELECT status INTO v_existing FROM typing_game.tournament_participants
  WHERE tournament_id = p_tournament AND participant_id = v_pid;
  IF FOUND THEN
    IF v_existing = 'active' THEN
      RAISE EXCEPTION 'DUPLICATE';
    END IF;
    UPDATE typing_game.tournament_participants SET status = 'active',
  ...
  IF v_cap IS NOT NULL THEN
    SELECT count(*)::int INTO v_count FROM typing_game.tournament_participants
    WHERE tournament_id = p_tournament AND status = 'active';
    IF v_count >= v_cap THEN
      RAISE EXCEPTION 'PARTICIPANT_CAP';
    END IF;
  END IF;
  INSERT INTO typing_game.tournament_participants
...
CREATE OR REPLACE FUNCTION typing_game.fn_withdraw_tournament(
  p_tournament uuid, p_clan uuid DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
...
BEGIN
  ...
  FROM typing_game.tournaments WHERE id = p_tournament;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  ...
  ELSE
    ...
    v_pid := p_clan;
  END IF;
  UPDATE typing_game.tournament_participants SET status = 'withdrawn'
  WHERE tournament_id = p_tournament AND participant_id = v_pid
    AND status = 'active';
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_seed_tournament(
...
BEGIN
  ...
  IF p_method = 'manual' THEN
    ...
    IF EXISTS (SELECT 1 FROM unnest(v_ids) AS x
               WHERE NOT EXISTS (
                 SELECT 1 FROM typing_game.tournament_participants p
                 WHERE p.tournament_id = p_tournament
                   AND p.participant_id = x AND p.status = 'active')) THEN
      RAISE EXCEPTION 'MALFORMED';
    END IF;
    IF (SELECT count(*) FROM typing_game.tournament_participants
        WHERE tournament_id = p_tournament AND status = 'active')
  ...
  ELSIF p_method = 'season_ranking' THEN
    ...
    SELECT v_ids || COALESCE(array_agg(p.participant_id ORDER BY p.participant_id), '{}')
      INTO v_ids
    FROM typing_game.tournament_participants p
  ...
  INSERT INTO typing_game.tournament_seeding
  ...
  FOR v_r IN 1 .. v_rounds LOOP
    ...
    INSERT INTO typing_game.tournament_rounds (tournament_id, round_no, name)
  ...
  FOR v_s IN 1 .. (v_slots / 2) LOOP
    ...
    SELECT id INTO v_round FROM typing_game.tournament_rounds
    WHERE tournament_id = p_tournament AND round_no = 1;
    IF v_a IS NOT NULL AND v_b IS NOT NULL THEN
      INSERT INTO typing_game.tournament_matches
      ...
      VALUES (p_tournament, v_round, v_s, v_a, v_b, 'pending',
        CASE WHEN (v_t.scoring_policy ->> 'source') = 'competition'
             THEN 'competition'::typing_game.tournament_match_source
             ELSE 'clan_war'::typing_game.tournament_match_source END)
      RETURNING id INTO v_match;
      INSERT INTO typing_game.tournament_match_participants
    ...
    ELSE
      ...
      INSERT INTO typing_game.tournament_adjustments
    ...
    FOR v_s IN 1 .. (v_slots / power(2, v_r)::int) LOOP
      ...
      SELECT p_tournament, v_round, v_s, ca.id, cb.id, 'pending',
        CASE WHEN (v_t.scoring_policy ->> 'source') = 'competition'
             THEN 'competition'::typing_game.tournament_match_source
             ELSE 'clan_war'::typing_game.tournament_match_source END
      FROM (SELECT m.id FROM typing_game.tournament_matches m
            JOIN typing_game.tournament_rounds r ON r.id = m.round_id
            WHERE m.tournament_id = p_tournament AND r.round_no = v_r - 1
              AND m.slot = 2 * v_s - 1) AS ca,
           (SELECT m.id FROM typing_game.tournament_matches m
            JOIN typing_game.tournament_rounds r ON r.id = m.round_id
            WHERE m.tournament_id = p_tournament AND r.round_no = v_r - 1
  ...
  FOR v_row IN
    ...
    FROM typing_game.tournament_matches m
    JOIN typing_game.tournament_rounds r ON r.id = m.round_id
    WHERE m.tournament_id = p_tournament AND r.round_no = 1
      AND m.status = 'bye'
  LOOP
    SELECT m.id INTO v_parent FROM typing_game.tournament_matches m
    JOIN typing_game.tournament_rounds r ON r.id = m.round_id
    ...
    IF FOUND THEN
      ...
      IF v_side = 'a' THEN
        UPDATE typing_game.tournament_matches SET participant_a = v_row.w
        WHERE id = v_parent;
      ELSE
        UPDATE typing_game.tournament_matches SET participant_b = v_row.w
        WHERE id = v_parent;
      END IF;
      INSERT INTO typing_game.tournament_match_participants
        (match_id, participant_id, side)
  ...
  UPDATE typing_game.tournaments SET status = 'seeded', updated_at = now()
  WHERE id = p_tournament;
  PERFORM typing_game.fn_log_tournament(
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_tmatch_decide(
...
CREATE OR REPLACE FUNCTION typing_game.fn_start_tournament(p_tournament uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
BEGIN
  PERFORM typing_game.fn_require_tournament_admin(p_tournament);
  UPDATE typing_game.tournaments SET status = 'live', updated_at = now()
  WHERE id = p_tournament AND status = 'seeded';
  IF NOT FOUND THEN
    RAISE EXCEPTION 'INVALID_STATE';
  END IF;
  UPDATE typing_game.tournament_matches m SET status = 'ready', updated_at = now()
  FROM typing_game.tournament_rounds r
...
CREATE OR REPLACE FUNCTION typing_game.fn_open_tmatch(p_match uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  v_t uuid;
BEGIN
  SELECT tournament_id INTO v_t FROM typing_game.tournament_matches
  WHERE id = p_match;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  PERFORM typing_game.fn_require_tournament_admin(v_t);
  UPDATE typing_game.tournament_matches SET status = 'live', updated_at = now()
...
CREATE OR REPLACE FUNCTION typing_game.fn_finalize_tmatch(
...
BEGIN
  ...
  SELECT * INTO v_m FROM typing_game.tournament_matches
  WHERE id = p_match FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  SELECT * INTO v_t FROM typing_game.tournaments WHERE id = v_m.tournament_id;
  PERFORM typing_game.fn_require_tournament_admin(v_m.tournament_id);
  ...
  IF v_m.source_type = 'clan_war' AND v_m.source_id IS NOT NULL THEN
    SELECT status::text INTO v_war_status FROM typing_game.clan_wars
    ...
    IF v_sa IS NULL OR v_sb IS NULL THEN
      SELECT total_score INTO v_sa FROM typing_game.clan_war_results
      WHERE war_id = v_m.source_id AND clan_id = v_m.participant_a;
      SELECT total_score INTO v_sb FROM typing_game.clan_war_results
  ...
  FROM typing_game.fn_tmatch_decide(
  ...
  UPDATE typing_game.tournament_matches SET
  ...
  SELECT * INTO v_parent FROM typing_game.tournament_matches
  ...
  IF FOUND THEN
    ...
    IF EXISTS (SELECT 1 FROM typing_game.tournament_matches m
    ...
    IF v_occupant IS NULL THEN
      IF v_side = 'a' THEN
        UPDATE typing_game.tournament_matches SET participant_a = v_win
        WHERE id = v_parent.id;
      ELSE
        UPDATE typing_game.tournament_matches SET participant_b = v_win
        WHERE id = v_parent.id;
      END IF;
      INSERT INTO typing_game.tournament_match_participants
        (match_id, participant_id, side)
...
CREATE OR REPLACE FUNCTION typing_game.fn_advance_tournament(p_tournament uuid)
RETURNS text
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  PERFORM typing_game.fn_require_tournament_admin(p_tournament);
  SELECT status INTO v_status FROM typing_game.tournaments
  WHERE id = p_tournament;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  ...
  IF v_open IS NULL THEN
    UPDATE typing_game.tournaments SET status = 'processing', updated_at = now()
    WHERE id = p_tournament;
    PERFORM typing_game.fn_log_tournament(
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_finalize_tournament(p_tournament uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  IF v_t.status <> 'processing' THEN
    RAISE EXCEPTION 'INVALID_STATE';
  END IF;
  IF EXISTS (SELECT 1 FROM typing_game.tournament_matches
             WHERE tournament_id = p_tournament
               AND status NOT IN ('finalized', 'bye', 'cancelled')) THEN
    RAISE EXCEPTION 'INVALID_STATE';
  END IF;
  SELECT max(rd.round_no)::int INTO v_rounds
  FROM typing_game.tournament_matches tm
  JOIN typing_game.tournament_rounds rd ON rd.id = tm.round_id
  ...
  INSERT INTO typing_game.tournament_results
    (tournament_id, participant_id, display_name, placement, details)
  VALUES (p_tournament, v_champion,
    COALESCE((SELECT display_name FROM typing_game.tournament_participants
              WHERE tournament_id = p_tournament
  ...
  FOR r IN
    SELECT DISTINCT tm.loser_id AS pid,
           (SELECT round_no FROM typing_game.tournament_rounds WHERE id = tm.round_id) AS rnd
    FROM typing_game.tournament_matches tm
  ...
  FOR r IN SELECT * FROM typing_game.tournament_results
           WHERE tournament_id = p_tournament
  LOOP
    ...
    IF v_t.participant_type = 'student' THEN
      ...
      INSERT INTO typing_game.tournament_reward_events
      ...
      IF FOUND THEN
        INSERT INTO typing_game.reward_events (key, user_id, attempt_id)
        ...
        IF v_xp > 0 THEN
          ...
          FROM typing_game.profiles WHERE id = r.participant_id
          ON CONFLICT (user_id, source, reference_id) DO NOTHING;
          UPDATE typing_game.profiles SET xp_total = xp_total + v_xp,
            current_level = (SELECT max(level) FROM typing_game.levels
                             WHERE required_xp <= xp_total + v_xp)
          WHERE id = r.participant_id AND EXISTS (
            SELECT 1 FROM typing_game.xp_ledger
        ...
        IF v_coins > 0 THEN
          ...
          FROM typing_game.profiles WHERE id = r.participant_id
          ON CONFLICT (user_id, source, reference_id) DO NOTHING;
          UPDATE typing_game.profiles SET coin_balance = coin_balance + v_coins
          WHERE id = r.participant_id AND EXISTS (
            SELECT 1 FROM typing_game.coin_ledger
            WHERE user_id = r.participant_id AND reference_id = v_key);
        END IF;
        v_n := v_n + 1;
      END IF;
    ELSE
      ...
      END IF;
      FOR m IN SELECT user_id FROM typing_game.clan_members
          ...
          IF v_xp > 0 THEN
            ...
            SELECT m.user_id, v_xp, 'tournament', 'tournament', v_key,
                   ...
                   jsonb_build_object('tournament_id', p_tournament,
                                      ...
                                      'placement', r.placement),
                   COALESCE(xp_total, 0) + v_xp
            FROM typing_game.profiles WHERE id = m.user_id
            ON CONFLICT (user_id, source, reference_id) DO NOTHING;
            UPDATE typing_game.profiles SET xp_total = xp_total + v_xp,
              current_level = (SELECT max(level) FROM typing_game.levels
                               WHERE required_xp <= xp_total + v_xp)
  ...
  UPDATE typing_game.tournaments SET version = version + 1 WHERE id = p_tournament;
  INSERT INTO typing_game.tournament_versions (tournament_id, version, definition)
  SELECT p_tournament, version,
         ...
         || jsonb_build_object('standings',
              (SELECT COALESCE(jsonb_agg(jsonb_build_object(
               ...
               FROM typing_game.tournament_results
               WHERE tournament_id = p_tournament))
  FROM typing_game.tournaments t WHERE t.id = p_tournament;
  UPDATE typing_game.tournaments
...
-- ---------------------------------------------------------------------------
ALTER TABLE typing_game.season_source_rules
...
CREATE OR REPLACE FUNCTION typing_game.fn_sync_tournament_season(
...
BEGIN
  ...
  SELECT status, start_at, end_at, scoring_policy
    INTO v_s FROM typing_game.seasons WHERE id = p_season;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  IF v_s.status NOT IN ('active', 'processing') THEN
    RETURN 0;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM typing_game.season_source_rules
  ...
  LOOP
    ...
    IF NOT EXISTS (SELECT 1 FROM typing_game.season_participants
    ...
    v_key := 'season:' || p_season::text || ':source:TOURNAMENT:'
      ...
      || ':v1';
    INSERT INTO typing_game.season_point_events
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_tournament_sweep()
RETURNS int
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  ...
  v_n int := 0;
BEGIN
  FOR r IN SELECT id FROM typing_game.tournaments
  ...
  LOOP
    UPDATE typing_game.tournaments SET status = 'live', updated_at = now()
    WHERE id = r.id;
    UPDATE typing_game.tournament_matches m SET status = 'ready',
      updated_at = now()
    FROM typing_game.tournament_rounds rd
    WHERE m.round_id = rd.id AND m.tournament_id = r.id
      AND rd.round_no = 1 AND m.status = 'pending'
      AND m.participant_a IS NOT NULL AND m.participant_b IS NOT NULL;
    PERFORM typing_game.fn_log_tournament(r.id, 'seeded', 'live', NULL);
    v_n := v_n + 1;
  END LOOP;
  RETURN v_n;
END;
...
CREATE OR REPLACE FUNCTION typing_game.fn_tournament_adjust(
...
BEGIN
  ...
  IF v_out NOT IN (v_m.participant_a, v_m.participant_b) THEN
    RAISE EXCEPTION 'MALFORMED';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM typing_game.tournament_participants
                 WHERE tournament_id = p_tournament
                   AND participant_id = v_in
                   AND status IN ('active', 'withdrawn')) THEN
    RAISE EXCEPTION 'INELIGIBLE';
  END IF;
  IF EXISTS (SELECT 1 FROM typing_game.tournament_seeding
             WHERE tournament_id = p_tournament
               AND participant_id = v_in) THEN
    RAISE EXCEPTION 'DUPLICATE';
  END IF;
  SELECT seed INTO v_seed FROM typing_game.tournament_seeding
  WHERE tournament_id = p_tournament AND participant_id = v_out;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  DELETE FROM typing_game.tournament_seeding
  WHERE tournament_id = p_tournament AND participant_id = v_out;
  INSERT INTO typing_game.tournament_seeding
    (tournament_id, participant_id, seed, source, snapshot_ref)
  ...
  IF v_m.participant_a = v_out THEN
    UPDATE typing_game.tournament_matches SET participant_a = v_in
    WHERE id = p_ref;
  ELSE
    UPDATE typing_game.tournament_matches SET participant_b = v_in
    WHERE id = p_ref;
  END IF;
  DELETE FROM typing_game.tournament_match_participants
  ...
  IF v_m.winner_id = v_out THEN
    UPDATE typing_game.tournament_matches SET winner_id = v_in WHERE id = p_ref;
  END IF;
  UPDATE typing_game.tournament_participants SET status = 'withdrawn'
  WHERE tournament_id = p_tournament AND participant_id = v_out;
  UPDATE typing_game.tournament_participants SET status = 'active'
...
REVOKE ALL ON FUNCTION typing_game.fn_log_tournament(uuid, typing_game.tournament_status, typing_game.tournament_status, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_log_tournament(uuid, typing_game.tournament_status, typing_game.tournament_status, uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_require_tournament_admin(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_require_tournament_admin(uuid, uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_create_tournament(jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_create_tournament(jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_update_tournament_draft(uuid, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_update_tournament_draft(uuid, jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_publish_tournament(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_publish_tournament(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_close_registration(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_close_registration(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_cancel_tournament(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_cancel_tournament(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_register_tournament(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_register_tournament(uuid, uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_withdraw_tournament(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_withdraw_tournament(uuid, uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_seed_tournament(uuid, text, uuid[], int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_seed_tournament(uuid, text, uuid[], int) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_start_tournament(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_start_tournament(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_open_tmatch(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_open_tmatch(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_tmatch_decide(uuid, uuid, uuid, numeric, numeric, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_tmatch_decide(uuid, uuid, uuid, numeric, numeric, jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_finalize_tmatch(uuid, numeric, numeric, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_finalize_tmatch(uuid, numeric, numeric, jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_advance_tournament(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_advance_tournament(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_finalize_tournament(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_finalize_tournament(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_sync_tournament_season(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_sync_tournament_season(uuid, uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_tournament_sweep() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_tournament_sweep() TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_tournament_adjust(uuid, text, uuid, jsonb, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_tournament_adjust(uuid, text, uuid, jsonb, text) TO authenticated;
