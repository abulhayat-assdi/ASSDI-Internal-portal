...
CREATE OR REPLACE FUNCTION typing_game.fn_log_season(
  p_season uuid, p_from typing_game.season_status,
  p_to typing_game.season_status, p_actor uuid
)
RETURNS void
LANGUAGE sql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
  INSERT INTO typing_game.season_state_events
    (season_id, from_status, to_status, actor_user_id)
  VALUES (p_season, p_from, p_to, p_actor);
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_create_season(p_def jsonb)
RETURNS uuid
...
BEGIN
  ...
  END IF;
  INSERT INTO typing_game.seasons (
  ...
  INSERT INTO typing_game.season_versions (season_id, version, definition)
  SELECT v_id, version, to_jsonb(s.*) - 'created_at' - 'updated_at'
  FROM typing_game.seasons s WHERE s.id = v_id;
  RETURN v_id;
...
CREATE OR REPLACE FUNCTION typing_game.fn_update_season_draft(
...
BEGIN
  IF NOT typing_game.fn_is_mission_admin() AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM typing_game.seasons
                 WHERE id = p_season AND status = 'draft') THEN
    RAISE EXCEPTION 'NOT_DRAFT';
  END IF;
  UPDATE typing_game.seasons SET
  ...
  FROM typing_game.seasons s WHERE s.id = p_season;
END;
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_set_season_source(
...
BEGIN
  IF NOT typing_game.fn_is_mission_admin() AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM typing_game.seasons WHERE id = p_season) THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  INSERT INTO typing_game.season_source_rules
...
CREATE OR REPLACE FUNCTION typing_game.fn_set_season_tier(
...
BEGIN
  IF NOT typing_game.fn_is_mission_admin() AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  INSERT INTO typing_game.season_tier_definitions
...
CREATE OR REPLACE FUNCTION typing_game.fn_schedule_season(p_season uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
BEGIN
  IF NOT typing_game.fn_is_mission_admin() AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  UPDATE typing_game.seasons SET status = 'scheduled', updated_at = now()
  WHERE id = p_season AND status = 'draft';
  IF NOT FOUND THEN
    RAISE EXCEPTION 'INVALID_STATE';
  END IF;
  PERFORM typing_game.fn_log_season(p_season, 'draft', 'scheduled', auth.uid());
END;
$$;

-- Freeze eligible participants (students + clans) at season start.
CREATE OR REPLACE FUNCTION typing_game.fn_season_freeze(p_season uuid)
RETURNS int
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  FROM typing_game.seasons WHERE id = p_season;
  SELECT COALESCE(array_agg(x::uuid) FILTER (WHERE x ~ '^[0-9a-fA-F-]{36}$'), '{}')
    INTO v_courses
  FROM jsonb_array_elements_text(
    (SELECT COALESCE(eligibility -> 'course_ids', '[]'::jsonb)
     FROM typing_game.seasons WHERE id = p_season)) AS x;
  INSERT INTO typing_game.season_participants
  ...
  JOIN LATERAL (
    SELECT bm.batch_id, b.course_id FROM typing_game.batch_members bm
...
CREATE OR REPLACE FUNCTION typing_game.fn_activate_season(p_season uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  IF v_overlap = 'GLOBAL_SINGLE' AND EXISTS (
      SELECT 1 FROM typing_game.seasons
      WHERE id <> p_season AND status = 'active') THEN
    RAISE EXCEPTION 'OVERLAP_FORBIDDEN';
  END IF;
  UPDATE typing_game.seasons SET status = 'active', updated_at = now()
  WHERE id = p_season AND status = 'scheduled';
  IF NOT FOUND THEN
    RAISE EXCEPTION 'INVALID_STATE';
  END IF;
  PERFORM typing_game.fn_season_freeze(p_season);
  PERFORM typing_game.fn_log_season(p_season, 'scheduled', 'active', auth.uid());
END;
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_cancel_season(p_season uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  v_status typing_game.season_status;
BEGIN
  IF NOT typing_game.fn_is_mission_admin() AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  SELECT status INTO v_status FROM typing_game.seasons WHERE id = p_season;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  IF v_status NOT IN ('draft', 'scheduled') THEN
    RAISE EXCEPTION 'INVALID_STATE';
  END IF;
  UPDATE typing_game.seasons SET status = 'cancelled', updated_at = now()
  WHERE id = p_season;
  PERFORM typing_game.fn_log_season(p_season, v_status, 'cancelled', auth.uid());
END;
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_join_season(p_season uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  IF NOT FOUND AND NOT EXISTS (
      SELECT 1 FROM typing_game.season_participants
...
CREATE OR REPLACE FUNCTION typing_game.fn_record_season_points(
...
BEGIN
  ...
  IF p_source = 'COMPETITION' AND p_type = 'student' THEN
    SELECT rank INTO v_rank FROM typing_game.competition_results
    WHERE competition_id = p_source_id AND scope = 'participant'
      AND ref_id = p_pid;
    IF NOT FOUND THEN
      RETURN false;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM typing_game.competitions
  ...
  ELSIF p_source = 'COMPETITION' AND p_type = 'clan' THEN
    ...
    FROM typing_game.competition_results res
    JOIN typing_game.competition_entries e
      ON e.competition_id = res.competition_id AND e.user_id = res.ref_id
    JOIN typing_game.clans cl ON cl.batch_id = e.batch_id
  ...
  ELSIF p_source = 'CLAN_WAR' AND p_type = 'clan' THEN
    SELECT is_winner INTO v_win FROM typing_game.clan_war_results
    WHERE war_id = p_source_id AND clan_id = p_pid;
    IF NOT FOUND THEN
      RETURN false;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM typing_game.clan_wars
                   WHERE id = p_source_id AND status = 'finalized') THEN
      RETURN false;
    END IF;
  ...
  ELSIF p_source = 'CLAN_BOSS' AND p_type = 'clan' THEN
    ...
    FROM typing_game.boss_instances i
    LEFT JOIN typing_game.boss_results br ON br.instance_id = i.id
  ...
  ELSIF p_source = 'CLAN_BOSS' AND p_type = 'student' THEN
    IF NOT EXISTS (SELECT 1 FROM typing_game.boss_participants p
                   JOIN typing_game.boss_instances i ON i.id = p.instance_id
  ...
  ELSIF p_source = 'MISSION' AND p_type = 'student' THEN
    IF NOT EXISTS (SELECT 1 FROM typing_game.mission_instances
  ...
  FROM typing_game.season_source_rules
  WHERE season_id = p_season AND source_type = p_source;
  IF v_cap > 0 THEN
    SELECT COALESCE(sum(points), 0)::int INTO v_sofar
    FROM typing_game.season_point_events
    WHERE season_id = p_season AND source_type = p_source
...
CREATE OR REPLACE FUNCTION typing_game.fn_sync_season(p_season uuid)
RETURNS int
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM typing_game.season_source_rules
             WHERE season_id = p_season AND source_type = 'COMPETITION'
               AND enabled) THEN
    FOR r IN
      SELECT c.id AS comp_id, c.ends_at, res.ref_id AS user_id, res.rank
      FROM typing_game.competitions c
      JOIN typing_game.competition_results res ON res.competition_id = c.id
    ...
    LOOP
      v_pts := CASE r.rank
        ...
        WHEN 3 THEN COALESCE((v_cfg -> 'COMPETITION' ->> 'third')::int, 40)
        ELSE COALESCE((v_cfg -> 'COMPETITION' ->> 'participation')::int, 10) END;
      IF typing_game.fn_record_season_points(
  ...
  IF EXISTS (SELECT 1 FROM typing_game.season_source_rules
    ...
    FOR r IN
      ...
      FROM typing_game.clan_wars w
      JOIN typing_game.clan_war_results res ON res.war_id = w.id
  ...
  IF EXISTS (SELECT 1 FROM typing_game.season_source_rules
    ...
    FOR r IN
      SELECT i.id AS inst_id, i.finalized_at, i.clan_id,
             (SELECT status FROM typing_game.boss_instances WHERE id = i.id) AS st
      FROM typing_game.boss_instances i
      WHERE i.status = 'finalized'
        AND i.finalized_at >= v_start AND i.finalized_at < v_end
    LOOP
      v_pts := CASE WHEN r.st = 'finalized'
          AND EXISTS (SELECT 1 FROM typing_game.boss_results br
    ...
    FOR r IN
      ...
      FROM typing_game.boss_instances i
      JOIN typing_game.boss_participants p ON p.instance_id = i.id
  ...
  IF EXISTS (SELECT 1 FROM typing_game.season_source_rules
    ...
    FOR r IN
      ...
      FROM typing_game.mission_instances i
...
CREATE OR REPLACE FUNCTION typing_game.fn_season_leaderboard(
...
BEGIN
  ...
  WITH totals AS (
    ...
    FROM typing_game.season_point_events e
  ...
  SELECT r.rnk::int, r.participant_id,
         COALESCE((SELECT p.display_name
                   FROM typing_game.season_participants p
                   WHERE p.season_id = p_season
                     AND p.participant_type = p_type
                     AND p.participant_id = r.participant_id),
                  'Player'),
         r.pts,
         (SELECT td.tier FROM typing_game.season_tier_definitions td
...
CREATE OR REPLACE FUNCTION typing_game.fn_finalize_season(p_season uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  END IF;
  PERFORM typing_game.fn_season_freeze(p_season);
  PERFORM typing_game.fn_sync_season(p_season);
  SELECT reward_policy INTO v_reward FROM typing_game.seasons WHERE id = p_season;
  FOR r IN SELECT * FROM typing_game.fn_season_leaderboard(p_season, 'student', 1000000)
  LOOP
    v_tier := r.tier;
    INSERT INTO typing_game.season_results
      (season_id, participant_type, participant_id, display_name,
    ...
    END IF;
    INSERT INTO typing_game.season_reward_events
    ...
    END IF;
  END LOOP;
  FOR r IN SELECT * FROM typing_game.fn_season_leaderboard(p_season, 'clan', 1000000)
  LOOP
    INSERT INTO typing_game.season_results
      (season_id, participant_type, participant_id, display_name,
       points, rank, tier, details)
    ...
    ON CONFLICT (key) DO NOTHING;
  END LOOP;
  INSERT INTO typing_game.season_leaderboard_snapshots
  ...
  LEFT JOIN LATERAL (
    SELECT * FROM typing_game.fn_season_leaderboard(p_season, t.ptype, 1000000)
  ) AS b ON true
  GROUP BY t.ptype
  ON CONFLICT (season_id, participant_type) DO NOTHING;
  UPDATE typing_game.seasons
  SET status = 'finalized', finalized_at = now(), updated_at = now()
  WHERE id = p_season;
  PERFORM typing_game.fn_log_season(p_season, 'processing', 'finalized', auth.uid());
  RETURN jsonb_build_object('season_id', p_season, 'rewards', v_n);
END;
$$;

-- Audited corrections only (history is never silently rewritten).
CREATE OR REPLACE FUNCTION typing_game.fn_season_adjust(
...
BEGIN
  IF NOT typing_game.fn_is_mission_admin() AND NOT typing_game.is_super_admin() THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  SELECT to_jsonb(r.*) INTO v_old FROM typing_game.season_results r
  WHERE season_id = p_season AND participant_type = p_type
    AND participant_id = p_pid;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  INSERT INTO typing_game.season_adjustments
...
CREATE OR REPLACE FUNCTION typing_game.fn_season_sweep()
RETURNS int
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  ...
  v_n int := 0;
BEGIN
  FOR r IN SELECT id FROM typing_game.seasons WHERE status = 'scheduled'
  LOOP
    BEGIN
      PERFORM typing_game.fn_activate_season_sweep(r.id);
      v_n := v_n + 1;
    EXCEPTION WHEN OTHERS THEN
      NULL;
    END;
  END LOOP;
  FOR r IN SELECT id FROM typing_game.seasons
           WHERE status = 'active' AND end_at <= now()
  LOOP
    UPDATE typing_game.seasons SET status = 'processing', updated_at = now()
    WHERE id = r.id;
    PERFORM typing_game.fn_log_season(r.id, 'active', 'processing', NULL);
    PERFORM typing_game.fn_finalize_season(r.id);
    v_n := v_n + 1;
  END LOOP;
  RETURN v_n;
END;
...
CREATE OR REPLACE FUNCTION typing_game.fn_activate_season_sweep(p_season uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  WHERE id = p_season;
  PERFORM typing_game.fn_season_freeze(p_season);
  PERFORM typing_game.fn_log_season(p_season, 'scheduled', 'active', NULL);
END;
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_advance_season(p_season uuid)
RETURNS typing_game.season_status
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
...
BEGIN
  ...
  IF v_status = 'scheduled' THEN
    PERFORM typing_game.fn_activate_season_sweep(p_season);
  ELSIF v_status = 'active' THEN
    UPDATE typing_game.seasons SET status = 'processing', updated_at = now()
    WHERE id = p_season;
    PERFORM typing_game.fn_log_season(p_season, 'active', 'processing', auth.uid());
    PERFORM typing_game.fn_finalize_season(p_season);
  END IF;
  SELECT status INTO v_status FROM typing_game.seasons WHERE id = p_season;
  RETURN v_status;
END;
...
REVOKE ALL ON FUNCTION typing_game.fn_create_season(jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_create_season(jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_update_season_draft(uuid, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_update_season_draft(uuid, jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_set_season_source(uuid, text, boolean, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_set_season_source(uuid, text, boolean, jsonb) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_set_season_tier(uuid, text, int, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_set_season_tier(uuid, text, int, int) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_schedule_season(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_schedule_season(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_activate_season(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_activate_season(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_cancel_season(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_cancel_season(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_join_season(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_join_season(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_record_season_points(uuid, text, uuid, text, uuid, int, timestamptz) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_record_season_points(uuid, text, uuid, text, uuid, int, timestamptz) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_sync_season(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_sync_season(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_season_leaderboard(uuid, text, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_season_leaderboard(uuid, text, int) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_finalize_season(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_finalize_season(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_season_adjust(uuid, text, uuid, jsonb, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_season_adjust(uuid, text, uuid, jsonb, text) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_season_sweep() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_season_sweep() TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_advance_season(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_advance_season(uuid) TO authenticated;
