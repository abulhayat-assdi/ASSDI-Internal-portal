...
DO $$ BEGIN
  CREATE TYPE typing_game.mission_status AS ENUM ('draft', 'active', 'inactive');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE TYPE typing_game.mission_instance_status AS ENUM (
...
DO $$ BEGIN
  CREATE TYPE typing_game.mission_period AS ENUM ('daily', 'weekly', 'event');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.missions (
  ...
  status typing_game.mission_status NOT NULL DEFAULT 'draft',
...
CREATE TABLE IF NOT EXISTS typing_game.mission_versions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS typing_game.mission_objectives (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  mission_id uuid NOT NULL REFERENCES typing_game.missions (id) ON DELETE CASCADE,
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS typing_game.mission_instances (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  mission_id uuid NOT NULL REFERENCES typing_game.missions (id) ON DELETE CASCADE,
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  period typing_game.mission_period NOT NULL,
...
CREATE INDEX IF NOT EXISTS mission_instances_user_idx
  ON typing_game.mission_instances (user_id, period_start DESC);

-- Which validated attempts contributed (dedupe + audit).
CREATE TABLE IF NOT EXISTS typing_game.mission_progress (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  instance_id uuid NOT NULL REFERENCES typing_game.mission_instances (id)
    ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS typing_game.mission_completion_events (
...
CREATE TABLE IF NOT EXISTS typing_game.daily_mission_assignments (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
  day date NOT NULL,
  mission_id uuid NOT NULL REFERENCES typing_game.missions (id) ON DELETE CASCADE,
  instance_id uuid NOT NULL REFERENCES typing_game.mission_instances (id)
...
CREATE TABLE IF NOT EXISTS typing_game.weekly_challenges (
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS typing_game.mission_reward_events (
  key text PRIMARY KEY,
  instance_id uuid NOT NULL REFERENCES typing_game.mission_instances (id)
    ON DELETE CASCADE,
  user_id uuid NOT NULL REFERENCES typing_game.profiles (id) ON DELETE CASCADE,
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_is_mission_admin()
RETURNS boolean
...
CREATE OR REPLACE FUNCTION typing_game.fn_can_view_mission_student(p_student uuid)
RETURNS boolean
...
BEGIN
  ...
  IF typing_game.is_super_admin() THEN
    RETURN true;
  END IF;
  -- fn_is_mission_admin() alone cannot gate this: an 'admin' role is always
  -- scoped to exactly one organization_id (see 0002's user_roles_scope_ck),
  -- so it must not grant visibility into a student from a different org.
  -- Only an admin of p_student's own organization (or their batch teacher)
  -- may view them.
  RETURN EXISTS (
    SELECT 1 FROM typing_game.batch_members m
    WHERE m.user_id = p_student
      AND (typing_game.is_teacher_of_batch(m.batch_id)
        OR typing_game.is_org_admin(typing_game.batch_organization_id(m.batch_id)))
  );
END;
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_mission_tz(p_user uuid)
RETURNS text
LANGUAGE sql STABLE SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
  SELECT COALESCE(NULLIF((SELECT timezone FROM typing_game.profiles WHERE id = p_user), ''), 'UTC');
$$;

-- Eligibility with human reasons (mirrors evaluateUnlock missing[]).
CREATE OR REPLACE FUNCTION typing_game.fn_mission_eligibility(
...
DECLARE
  ...
  v_badge text;
BEGIN
  SELECT prerequisites INTO v_pre FROM typing_game.missions WHERE id = p_mission;
  IF NOT FOUND THEN
  ...
  FOR v_slug IN SELECT * FROM jsonb_array_elements_text(COALESCE(v_pre -> 'games', '[]'::jsonb)) LOOP
    IF NOT EXISTS (
      SELECT 1 FROM typing_game.game_unlocks
  ...
  FOR v_slug IN SELECT * FROM jsonb_array_elements_text(COALESCE(v_pre -> 'missions', '[]'::jsonb)) LOOP
    IF NOT EXISTS (
      SELECT 1 FROM typing_game.mission_instances i
  ...
  FOR v_badge IN SELECT * FROM jsonb_array_elements_text(COALESCE(v_pre -> 'badges', '[]'::jsonb)) LOOP
    IF NOT EXISTS (
      SELECT 1 FROM typing_game.badge_awards
