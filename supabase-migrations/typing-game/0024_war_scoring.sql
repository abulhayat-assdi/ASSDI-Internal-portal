...
CREATE OR REPLACE FUNCTION typing_game.fn_submit_war_attempt(
...
BEGIN
  ...
  END IF;
  SELECT * INTO v_part FROM typing_game.clan_war_participants
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM typing_game.clan_war_games
             WHERE war_id = p_war)
     AND NOT EXISTS (SELECT 1 FROM typing_game.clan_war_games
  ...
  END IF;
  INSERT INTO typing_game.clan_war_attempts
  ...
  END IF;
  UPDATE typing_game.clan_war_participants
...
CREATE OR REPLACE FUNCTION typing_game.fn_sync_war(p_war uuid)
...
BEGIN
  ...
  FOR r IN SELECT user_id, clan_id FROM typing_game.clan_war_participants
           WHERE war_id = p_war AND eligible
  LOOP
    WITH mine AS (
      SELECT wa.score, res.effective_wpm AS wpm, res.accuracy AS acc
      FROM typing_game.clan_war_attempts wa
      JOIN typing_game.attempt_results res ON res.attempt_id = wa.attempt_id
    ...
    INSERT INTO typing_game.clan_war_contributions
...
CREATE OR REPLACE FUNCTION typing_game.fn_finalize_war(p_war uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
...
BEGIN
  ...
  END IF;
  PERFORM typing_game.fn_sync_war(p_war);
  v_mode := COALESCE(v_war.scoring_profile ->> 'mode', 'sum');
  v_topn := GREATEST(COALESCE((v_war.scoring_profile ->> 'top_n')::int, 3), 1);
  ...
  FOR v_row IN
    WITH per_clan AS (
      SELECT c.clan_id,
        ...
        (SELECT min(wa.submitted_at) FROM typing_game.clan_war_attempts wa
         WHERE wa.war_id = p_war AND wa.user_id IN (
           SELECT user_id FROM typing_game.clan_war_participants p2
  ...
  INSERT INTO typing_game.clan_war_results
  ...
  FOR v_uid IN SELECT user_id FROM typing_game.clan_war_contributions
               WHERE war_id = p_war AND attempts > 0
  LOOP
    IF EXISTS (SELECT 1 FROM typing_game.clan_war_participants p2
    ...
    INSERT INTO typing_game.clan_war_reward_events (key, war_id, user_id, xp, coins)
  ...
  WHERE id = p_war;
  PERFORM typing_game.fn_log_war(p_war, 'processing', 'finalized', auth.uid());
...
CREATE OR REPLACE FUNCTION typing_game.fn_war_board(p_war uuid)
RETURNS TABLE (
  scope text,
  clan_id uuid,
  display_name text,
...
DECLARE
  ...
  v_mine uuid;
BEGIN
  IF NOT typing_game.fn_is_war_viewer(p_war) THEN
    RETURN;
  END IF;
  ...
  FROM typing_game.clan_war_results r
  JOIN typing_game.clans c ON c.id = r.clan_id
  WHERE r.war_id = p_war
  UNION ALL
  SELECT 'clan_live'::text, cc.clan_id, c.name,
         COALESCE(sum(cc.score), 0), COALESCE(sum(cc.attempts), 0)::int, false
  FROM typing_game.clan_war_contributions cc
  JOIN typing_game.clans c ON c.id = cc.clan_id
  WHERE cc.war_id = p_war
    AND NOT EXISTS (SELECT 1 FROM typing_game.clan_war_results r2
  ...
  FROM typing_game.clan_war_contributions cc
  JOIN typing_game.profiles p ON p.id = cc.user_id
  WHERE cc.war_id = p_war AND cc.attempts > 0
    AND (cc.clan_id = v_mine OR cc.user_id = v_me
      OR cc.user_id IN (
        SELECT cc2.user_id FROM typing_game.clan_war_contributions cc2
...
CREATE OR REPLACE FUNCTION typing_game.fn_war_adjust(
...
BEGIN
  ...
  END IF;
  IF NOT typing_game.fn_can_manage_clan(v_war.challenger_clan_id)
    AND NOT typing_game.fn_can_manage_clan(v_war.defender_clan_id) THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  SELECT to_jsonb(c.*) INTO v_old FROM typing_game.clan_war_contributions c
  WHERE war_id = p_war AND user_id = p_ref;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  UPDATE typing_game.clan_war_contributions
  SET score = COALESCE((p_patch ->> 'score')::numeric, score)
  WHERE war_id = p_war AND user_id = p_ref;
  INSERT INTO typing_game.clan_war_adjustments
    (war_id, scope, ref_id, old_value, new_value, reason, created_by)
  VALUES (p_war, p_scope, p_ref, v_old,
    (SELECT to_jsonb(c.*) FROM typing_game.clan_war_contributions c
     WHERE war_id = p_war AND user_id = p_ref),
    p_reason, auth.uid());
END;
$$;

REVOKE ALL ON FUNCTION typing_game.fn_submit_war_attempt(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_submit_war_attempt(uuid, uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_sync_war(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_sync_war(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_finalize_war(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_finalize_war(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_war_board(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_war_board(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_war_adjust(uuid, text, uuid, jsonb, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_war_adjust(uuid, text, uuid, jsonb, text) TO authenticated;
