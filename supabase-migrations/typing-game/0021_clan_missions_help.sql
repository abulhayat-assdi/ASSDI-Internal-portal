...
CREATE OR REPLACE FUNCTION typing_game.fn_link_clan_mission(
...
DECLARE
  v_id uuid;
BEGIN
  IF NOT typing_game.fn_can_manage_clan(p_clan) THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM typing_game.missions
                 WHERE id = p_mission AND status = 'active'
                   AND category = 'CLAN') THEN
    RAISE EXCEPTION 'NOT_CLAN_MISSION';
  END IF;
  INSERT INTO typing_game.clan_missions (mission_id, clan_id, period_start)
  VALUES (p_mission, p_clan, p_period_start)
  ON CONFLICT (mission_id, clan_id, period_start) DO NOTHING
  RETURNING id INTO v_id;
  IF NOT FOUND THEN
    SELECT id INTO v_id FROM typing_game.clan_missions
...
CREATE OR REPLACE FUNCTION typing_game.fn_start_clan_mission(p_id uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  v_clan uuid;
  v_status typing_game.mission_instance_status;
BEGIN
  SELECT clan_id, status INTO v_clan, v_status
  FROM typing_game.clan_missions WHERE id = p_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  IF NOT typing_game.fn_is_clan_staff(v_clan, auth.uid())
    AND NOT typing_game.fn_can_manage_clan(v_clan) THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  IF v_status <> 'available' THEN
    RAISE EXCEPTION 'INVALID_STATE';
  END IF;
  UPDATE typing_game.clan_missions
  SET status = 'active', started_at = now() WHERE id = p_id;
  PERFORM typing_game.fn_log_clan(v_clan, 'mission_started', auth.uid(),
...
CREATE OR REPLACE FUNCTION typing_game.fn_sync_clan_mission(p_id uuid)
...
DECLARE
  ...
  v_uid uuid;
BEGIN
  SELECT * INTO v_cm FROM typing_game.clan_missions WHERE id = p_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  IF v_cm.status <> 'active' THEN
    RETURN v_cm.status;
  END IF;
  SELECT * INTO v_m FROM typing_game.missions WHERE id = v_cm.mission_id;
  ...
  LOOP
    ...
    WITH base AS (
      ...
      JOIN typing_game.clan_members m ON m.user_id = a.user_id
      ...
      SELECT
        ...
        (SELECT count(*)::numeric FROM typing_game.personal_records pr
         JOIN typing_game.clan_members m2 ON m2.user_id = pr.user_id
    ...
    IF v_obj.kind = 'WPM_REACHED' AND (v_obj.target ->> 'count')::int IS NOT NULL THEN
      ...
      FROM (SELECT a.user_id FROM typing_game.game_attempts a
            JOIN typing_game.attempt_results r ON r.attempt_id = a.id
            JOIN typing_game.games g ON g.id = a.game_id
            JOIN typing_game.clan_members m ON m.user_id = a.user_id
            WHERE m.clan_id = v_cm.clan_id AND m.status = 'active'
  ...
  IF v_done = v_n THEN
    ...
    INSERT INTO typing_game.clan_mission_completion_events (key, clan_mission_id, clan_id)
      ...
      FOREACH v_uid IN ARRAY v_all_parts LOOP
        ...
        IF FOUND THEN
          IF v_xp > 0 THEN
            ...
            FROM typing_game.profiles WHERE id = v_uid
        ...
        END IF;
      END LOOP;
      PERFORM typing_game.fn_log_clan(v_cm.clan_id, 'mission_completed', NULL,
        jsonb_build_object('clan_mission_id', p_id,
          'contributors', COALESCE(array_length(v_all_parts, 1), 0)));
    END IF;
    RETURN 'completed'::typing_game.mission_instance_status;
  END IF;
  UPDATE typing_game.clan_missions SET progress = v_prog WHERE id = p_id;
  RETURN 'active'::typing_game.mission_instance_status;
END;
$$;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION typing_game.fn_expire_help_requests()
RETURNS int
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = typing_game, public, pg_temp
AS $$
DECLARE
  ...
  r record;
BEGIN
  FOR r IN SELECT id, clan_id FROM typing_game.clan_help_requests
           WHERE status IN ('open', 'partially_fulfilled') AND expires_at <= now()
  LOOP
    UPDATE typing_game.clan_help_requests SET status = 'expired' WHERE id = r.id;
    PERFORM typing_game.fn_log_clan(r.clan_id, 'help_expired', NULL,
...
CREATE OR REPLACE FUNCTION typing_game.fn_create_help_request(
...
DECLARE
  ...
  v_ttl int;
BEGIN
  PERFORM typing_game.fn_expire_help_requests();
  IF v_me IS NULL THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM typing_game.clan_members
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM typing_game.clan_help_requests
  ...
  INSERT INTO typing_game.clan_help_requests
  ...
  RETURNING id INTO v_id;
  PERFORM typing_game.fn_log_clan(p_clan, 'help_requested', v_me,
    jsonb_build_object('request_id', v_id, 'requested', p_requested));
  RETURN v_id;
END;
$$;

CREATE OR REPLACE FUNCTION typing_game.fn_contribute_help(
...
BEGIN
  ...
  IF v_me IS NULL THEN
    RAISE EXCEPTION 'FORBIDDEN';
  END IF;
  SELECT * INTO v_req FROM typing_game.clan_help_requests WHERE id = p_request
  FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'NOT_FOUND';
  END IF;
  ...
  IF p_amount IS NULL OR p_amount <= 0 THEN
    RAISE EXCEPTION 'MALFORMED';
  END IF;
  IF EXISTS (SELECT 1 FROM typing_game.clan_help_contributions
  ...
  FROM typing_game.clan_help_contributions c
  JOIN typing_game.clan_help_requests r ON r.id = c.request_id
  ...
  UPDATE typing_game.profiles SET coin_balance = coin_balance - v_new
  ...
  FROM typing_game.profiles WHERE id = v_req.requester_user_id
  ON CONFLICT (user_id, source, reference_id) DO NOTHING;
  UPDATE typing_game.profiles SET xp_total = xp_total + v_new,
    current_level = (SELECT max(level) FROM typing_game.levels
  ...
  INSERT INTO typing_game.clan_help_contributions
  ...
  UPDATE typing_game.clan_help_requests
  SET fulfilled = fulfilled + v_new,
      status = CASE WHEN fulfilled + v_new >= requested
                      THEN 'fulfilled'::typing_game.clan_help_status
                    ELSE 'partially_fulfilled'::typing_game.clan_help_status END
  WHERE id = p_request;
  PERFORM typing_game.fn_log_clan(v_req.clan_id,
...
REVOKE ALL ON FUNCTION typing_game.fn_link_clan_mission(uuid, uuid, date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_link_clan_mission(uuid, uuid, date) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_start_clan_mission(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_start_clan_mission(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_sync_clan_mission(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_sync_clan_mission(uuid) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_create_help_request(uuid, jsonb, int, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_create_help_request(uuid, jsonb, int, int) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_contribute_help(uuid, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_contribute_help(uuid, int) TO authenticated;
REVOKE ALL ON FUNCTION typing_game.fn_expire_help_requests() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION typing_game.fn_expire_help_requests() TO authenticated;
